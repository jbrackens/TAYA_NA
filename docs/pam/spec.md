...
| Positions & exposure | 15, 16 | 9 | **Built** | Pass | Trading core built (positions, exposure/P&L, house risk dashboard, per-market eligibility GAP-20, admin order view/cancel GAP-21); **+ per-PLAYER operator surface (GAP-89)** — admin positions read (`GET /admin/positions?userId=`, markets:read, `fd1653a7`) + Profile-360 "Positions & Orders" tab showing working orders (open+partial), open positions, and total exposure (`8172a0ae`), wiring the GAP-21 admin-orders route into the office. Scenario 9 (Market-Ops/Risk inspecting a trader's positions) is now operator-usable |
...
| Notes & comms | 20 | 12 | **Built** | Pass | Notes + notification-template store + admin editor (P1-6 `a2cf341f`); **sent-communication history + agent-initiated templated send (GAP-43 backend)** (`internal/communications`, audited `player.communication_sent`, rate-limited GAP-75); **+ Profile-360 Communications operator UI (GAP-90: history view `af95a3a2` + users:write-gated templated send `557529af`)** — an agent can now view a player's comm history and send a templated notice from the console. Minor: template `data` vars not yet operator-fillable (render verbatim) |
