...
|---|---|---|---|---|---|
| Admin auth & RBAC | 6, 7, 11, 27 | 1 | **Built** | Pass | MFA now **mandatory for admin roles** (P0-1 `6914421c`,`268157b8`), RBAC (mig 027/038/040) + permission-denial audit (GAP-25 `d719804f`), least-privilege personas (GAP-14 `bd0634db`), admin MFA-reset backend (GAP-15 `21743d22`), **admin session-revocation / kick-session (GAP-76 `7252cf97`)**. Residual: the MFA-reset **operator UI is pending (GAP-88)** — API-built, not yet a console control; WebAuthn/passkey option (GAP-71); guaranteed single-session "steal-lock" deferred (GAP-76 slice 2) |
