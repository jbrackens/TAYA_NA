...
> Research gathered 2026-07-12 for the P10 "Signal Ink" / P11 redesign, which the owner rejected; the branch
> is preserved at tag `archive/p10-signal-ink-2026-07-12`. The design is dead — this research is kept for its
> competitor, regulatory and dark-pattern findings, which outlived it. Every non-visual fix from that branch
> was re-implemented fresh on `main` and deployed on 2026-07-12. See `CLAUDE.md` for current architecture.
