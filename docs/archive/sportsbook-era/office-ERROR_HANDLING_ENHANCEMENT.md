...
> The ErrorBoundary / LoadingSpinner / SkeletonLoader / ErrorState components it documents do still
> exist in `office/app/components/shared/`, but the pages it wires them into (including `/trading`
> with fixtures and markets) were dropped with the sportsbook domain, and its API-integration phases
> were never executed as written.
