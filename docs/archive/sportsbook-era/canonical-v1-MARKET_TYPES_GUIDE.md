...
> It documented sportsbook market types, decimal-odds validation and sport settlement resolvers in
> `go-platform/modules/platform/canonical/v1/`; that code is still in the tree but has no caller
> anywhere in the platform, and the `sports_handlers.go` it claims to have updated does not exist.
> Prediction markets settle through `services/gateway/internal/prediction/settlement.go` instead.
