...
> A sportsbook-era sample-data integration progress log for the back-office; superseded by the
> prediction-admin, access-control, disputes and social-moderation routes under
> `office/app/(dashboard)/`, with `office/FEATURE_MANIFEST.json` as the status source of truth.
