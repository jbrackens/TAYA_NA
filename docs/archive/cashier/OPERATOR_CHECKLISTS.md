...
> Not executable: the backoffice /cashier page and the Alpha admin routes it drives no longer exist in the points-only build.
> See `CLAUDE.md` for current architecture.

# TapTrade Cashier Operator Checklists

**Status:** SUPERSEDED 2026-09-06 — abandoned workstream (was: "Stage 1 Alpha operational checklist.").
**Date:** 2026-05-28.
...
**Not executable.** This checklist assumes a backoffice `/cashier` page and the
`/api/v1/admin/cashier/alpha/*` routes. Neither exists in the points-only build:
`office/proxy.ts` 404s `/cashier` before auth,
`office/tests/app-router-legacy-routes.test.ts` asserts
`app/(dashboard)/cashier/page.tsx` stays absent, and the gateway registers the
alpha cashier routes only behind `TAPTRADE_LEGACY_MONEY_ROUTES_ENABLED`, which
is refused at boot in production and staging. The steps are kept as a record of
