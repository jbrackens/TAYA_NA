...
> The custodial USDC Alpha was never enabled; ALPHA_CASHIER_ENABLED=true is now refused at boot in production and staging.
> See `CLAUDE.md` for current architecture.

# TapTrade Custodial USDC Cashier Plan

**Status:** SUPERSEDED 2026-09-06 — abandoned workstream (was: "Closed Alpha and Beta execution plan.").
...
**OBSOLETE — do not follow.** This section originally named
`apps/taptrade-platform/phoenix-frontend-brand-viegg/packages/app-core/` as the
primary app area and listed cashier client, hook, component and locale files to
add or modify there. Both halves of that instruction are wrong now:

- `phoenix-frontend-brand-viegg/` is a preservation archive of a legacy
  frontend, not the live player app (see `ALLOWLIST.md`). Writing new code into
  it falsifies the preservation record.
- The live player app is
  `apps/taptrade-platform/frontend/packages/app/`, and it must not gain cashier
  UI. `app/__tests__/qa-regressions-2026-04-18.test.ts` fails if
  `app/cashier/*`, `components/CryptoDepositCard.tsx`,
  `components/NonCustodialCashierStatus.tsx`,
  `components/CashierCanaryPanel.tsx`,
  `lib/api/alpha-cashier-client.ts`, `lib/api/noncustodial-cashier-client.ts`
  or `lib/api/crypto-client.ts` reappear.

The list of intended player-facing behaviours is kept below as a record of what
the Alpha would have done. It is not a build instruction.

Player UX requirements (never built):
...
**OBSOLETE — do not follow.** This section told the implementer to add
`containers/provider-ops/cashier-review.tsx` and
`app/(dashboard)/cashier/page.tsx` to
`apps/taptrade-platform/frontend/packages/office/`. Both files are now asserted
absent by `office/tests/app-router-legacy-routes.test.ts`
("keeps launch-prohibited cashier/payment admin surfaces retired"), the same
test forbids a `/cashier` nav entry, and `office/proxy.ts` 404s the `/cashier`
path before authentication.

The list below is kept as a record of what the operator screen would have shown.
It is not a build instruction.

Backoffice UX requirements (never built):
...
- Operator docs/env samples: this plan, `docs/archive/cashier/README.md`, `apps/taptrade-platform/README.md`, `apps/taptrade-platform/DEVELOPMENT.md`, `apps/taptrade-platform/DEPLOYMENT.md`, and `apps/taptrade-platform/docker-compose.demo.yml`.
...
- Read this document.
- Read `docs/archive/cashier/README.md`.
