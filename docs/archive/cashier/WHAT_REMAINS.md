...
> The work did not pause for provider credentials — it was abandoned when the launch went points-only and real-money surfaces were classified prohibited.
> See `CLAUDE.md` for current architecture.

# TapTrade Cashier: What Remains

**Date:** 2026-05-25
**Status:** SUPERSEDED 2026-09-06 — abandoned workstream (was: "Paused pending provider credentials and final stack decisions.").
The work below did not stop because a provider credential was missing. It
stopped because the launch went points-only and every real-money surface was
classified launch-prohibited. Nothing here will be resumed.

One item that used to be listed as completed has been struck from the list:
"frontend status views for non-custodial deposit and withdrawal states". Those
views were deleted, and `app/__tests__/qa-regressions-2026-04-18.test.ts` now
fails if `components/NonCustodialCashierStatus.tsx`,
`components/CryptoDepositCard.tsx` or `components/CashierCanaryPanel.tsx` come
back. The work was reversed, not completed.
...
That result is not evidence of cashier readiness. `scripts/check-cashier-guards.sh`
was retargeted to assert the cashier's *absence* from the player app (it prints
"non-custodial cashier frontend client absent; TapTrade launch has no
user-facing cashier"), and the remaining checks only prove the archived fixtures,
schema and OpenAPI artifacts are internally consistent with each other.
