...
> The Rocket.Chat integration it plans was never built: the Go gateway has no chat package, none of the `/api/v1/chat/*` endpoints exist, and the `services/codex-prep/` scripts this file invokes are not in the repo. The player app ships a mock chat panel instead (`frontend/packages/app/app/components/chat/ChatSidebar.tsx`, `MOCK_CHAT_MESSAGES` — local-only, no server).
