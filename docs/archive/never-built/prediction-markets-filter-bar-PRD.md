...
> Never built. Every surface and file it targets (`/prediction/markets`, `packages/app-core/...`) lives in `apps/taptrade-platform/phoenix-frontend-brand-viegg`, a tree that is not built by any workflow and not deployed; the shipped player app has no `/prediction/markets` route and no `app-core` package. Discovery shipped instead as `/predict`, `/discover`, `/floor` and `/category/[slug]`, with the curated sections served by the gateway (`GET /api/v1/discovery` returns `featured` / `trending` / `closingSoon` / `recent`) rather than as URL-driven filter chips. The §5 color spec predates two rebrands.
> See `CLAUDE.md` for current architecture.

| | |
|---|---|
| **ID** | PMF-001 |
| **Status** | Draft — never built |
| **Date** | 2026-04-25 |
