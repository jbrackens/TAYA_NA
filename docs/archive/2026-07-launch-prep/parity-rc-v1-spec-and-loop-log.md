...
> This was `spec.md` at the repository root: the Parity Release Candidate v1 requirements
> plus 434 "Loop N evidence note" entries from an autonomous loop that stopped on
> 2026-07-01, and it was never a maintained specification after that date.
> Two things in it are actively wrong now. Its API contract names fields
> `pricePointsCents`, `reservedPointsCents`, `yesPriceCents` and the like — the
> "point-cents" nested-unit model that migration `050_points_unit_model.sql` deleted on
> 2026-07-07; the current test suite fails when those spellings appear on the wire, so
> implementing to this contract produces code the repo's own guardrails reject. And its
> §2 launch constraint "No fiat deposits" was superseded on 2026-07-12 by the point store,
> which sells point packs for USD (closed-loop: money in, non-redeemable points out, no
> cash-out) — see `STORE_AND_PAYMENTS.md`.
> Its still-accurate core — points-only, non-redeemable, no withdrawal or cash-out path —
> is stated in `README.md` and `CURRENT_STATE.md`; implemented journeys are in
