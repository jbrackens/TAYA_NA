...
> A single-session performance audit of the player app run on 2026-07-12; its work landed
> on `main` and its branch `feat/store-and-perf-audit` is gone. Every measurement here —
> Lighthouse scores, bundle sizes, route inventory — was taken against a July build that
> three later redesigns have superseded (Ink & lime step 2.5 on 2026-07-26, the Tap Path
> purple + gold identity on 2026-08-22, and the trending-moments rebuild of `/predict` on
> 2026-08-24), so none of the numbers are current baselines. Its §2 inventory line "no
> paid store exists anywhere" was true when written and was resolved by the store this
> same audit went on to build — see `STORE_AND_PAYMENTS.md`, which is still live.
