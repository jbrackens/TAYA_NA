...
```sh
CHAT_PUBLIC_URL=https://chat.staging.hulana.com \
CHAT_PARENT_ORIGIN=https://staging.hulana.com \
services/codex-prep/scripts/chat-origin-validate.sh
```

This must pass before public enablement. It requires a CSP header with
`frame-ancestors` that includes the Hula parent origin.
...
- Configure iframe embedding so the Hula app origin is allowed and no other
  origin is allowed.
- Confirm the Rocket.Chat response headers allow the Hula app origin to embed
  ...
  a tight `Content-Security-Policy: frame-ancestors 'self' https://<hula-app-origin>`.
- Confirm a same-site deployment plan: production should use a subdomain such
  as `chat.hulana.com`, not a third-party origin.
...
- Assign Hula `admin` and `moderator` mapped roles and verify delete, mute,
...
- Define deletion workflow across Rocket.Chat data and Hula audit records.
...
- Chat report creates a durable Hula audit row.
