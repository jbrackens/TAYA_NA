...
if rg -n '"/api/v1/payments/crypto/deposit-address"' "$APP_DIR/lib/api" "$APP_DIR/cashier"; then
...
if ! rg -n "TestCashierProviderCallbackBypassesAuthAndCSRFMiddleware" "$GATEWAY_DIR/cmd/gateway/main_test.go" >/dev/null; then
