...
class RewardsMarketingSettlementDataStream(brand: SourceBrand)(implicit configuration: SbTechConfig)
  extends KeyedFlatMapStream[SettlementData, SbTechConfig, Long, CasinoAndSportBetsTopupData, SourceBrand]((s"Rewards marketing settlement data ${brand.id}"), brand)
    with RewardsMarketingSettlementDataToTopupData
    with RewardsMarketingBetInfoDataFilter {

  override def groupId: String = s"rewards-marketing-settlement-data-stream-${brand.name}"
...
object RewardsMarketingSettlementDataStream {
  ...
  def execute(brand: SourceBrand)(implicit configuration: SbTechConfig): Unit = new RewardsMarketingSettlementDataStream(brand).execute()
