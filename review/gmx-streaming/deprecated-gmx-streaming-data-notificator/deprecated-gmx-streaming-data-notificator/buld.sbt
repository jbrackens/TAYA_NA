...
val settings = Seq(organization := "net.flipsports.gmx.streaming.internal.datalake.notifications",
...
lazy val `gmx-streaming-data-notifications` = (project in file("."))
  .aggregate(`notifications-jobs`)
  .settings(settings)

lazy val `notifications-jobs` = project
  .settings(settings)
  .settings(libraryDependencies ++= Deps.notificationJobs)
