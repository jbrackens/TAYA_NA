...
import net.flipsports.gmx.streaming.sbtech.filters.v1.RewardsMarketingBetInfoDataFilter
import net.flipsports.gmx.streaming.sbtech.mappers.v1.RewardsMarketingSettlementDataToTopupData
...
class RewardsMarketingSettlementDataStream(metaParameters: MetaParameters, businessMetaParameters: BusinessMetaParameters, configuration: SbTechConfig)
  ...
  val kafkaProperties: KafkaProperties = configuration.kafka.withGroupId(s"gmx-streaming.rewards-marketing-settlement-data-stream-${businessMetaParameters.brand().sourceBrand.name}")
  ...
  override def transform(dataStream: DataStream[FlinkTuple[SettlementDataCustomerId, SettlementData]])(implicit sourceTypeInformation: TypeInformation[FlinkTuple[SettlementDataCustomerId, SettlementData]], targetTypeInformation: TypeInformation[FlinkTuple[Long, CasinoAndSportBetsTopupData]]): DataStream[FlinkTuple[Long, CasinoAndSportBetsTopupData]] =
    dataStream.flatMap(new RewardsMarketingSettlementDataToTopupData(businessMetaParameters.brand()))
  ...
  override def filtersDefinition: InputOutputFilter[FlinkTuple[SettlementDataCustomerId, SettlementData], FlinkTuple[Long, CasinoAndSportBetsTopupData]] = new RewardsMarketingBetInfoDataFilter()

}


object RewardsMarketingSettlementDataStream {
  ...
  def execute(metaParameters: MetaParameters, businessMetaParameters: BusinessMetaParameters, config: SbTechConfig): Unit = new RewardsMarketingSettlementDataStream(metaParameters, businessMetaParameters, config).stream()
