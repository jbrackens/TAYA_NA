...
class AffiliatesFemaleRegistrationDownstreamSpec extends StreamingTestBase with FlinkJobsTestRunner {

  "Application" should {

    "process undecided items" in {
      val messages = CustomerDetailsDataProvider.asScalaAllWithCurrentRegistration(Some("customerdetails-affiliates.json")).map(i => new Tuple2(i._1.getCustomerID, i._2))
      ...
      val sink = new DownstreamSinkFunction() {
        override def write(value: Tuple2[KeyType, ValueType]): Unit = AffiliatesFemaleRegistrationDownstreamSpec.result.add(value)
      }
      AffiliatesFemaleRegistrationDownstream(metaParameters)
        .processStream(sourceStream, env)(env.getConfig)
        .addSink(sink)

      env.execute()

      AffiliatesFemaleRegistrationDownstreamSpec.result.size() must be(1)
      AffiliatesFemaleRegistrationDownstreamSpec.result.get(0).f0.getExternalUserId.toString must be ("1")
    }

  }
}

object AffiliatesFemaleRegistrationDownstreamSpec {
