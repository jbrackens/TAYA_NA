...
class FemaleMobileCasinoRegistrationDownstreamSpec extends StreamingTestBase with FlinkJobsTestRunner {
  ...
  "Application" should {
    ...
    "process undecided items" in {
      ...
      val sink = new DownstreamSinkFunction() {
        override def write(value: Tuple2[KeyType, ValueType]): Unit = FemaleMobileCasinoRegistrationDownstreamSpec.result.add(value)
      }
      FemaleMobileCasinoRegistrationDownstream(metaParameters)
        .processStream(sourceStream, env)(env.getConfig)
        .addSink(sink)

      env.execute()

      FemaleMobileCasinoRegistrationDownstreamSpec.result.size() must be(1)
    }

  }
}

object FemaleMobileCasinoRegistrationDownstreamSpec {
