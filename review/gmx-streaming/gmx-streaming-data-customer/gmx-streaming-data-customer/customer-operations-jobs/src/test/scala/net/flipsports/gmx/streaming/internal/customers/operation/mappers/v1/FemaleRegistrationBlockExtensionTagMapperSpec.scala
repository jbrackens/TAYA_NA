...
class FemaleRegistrationBlockExtensionTagMapperSpec extends StreamingTestBase {

  val mapper = new FemaleRegistrationBlockExtensionTagMapper(Brand.sportNations)

  "Female account registration" should {
    val source = CustomerWithLoginDataProvider.single
    source.f1.customerDetail.setGender(Gender.Female.toString)
