...
class IrishRegistrationDownstreamSpec extends StreamingTestBase with FlinkJobsTestRunner {
  ...
  "Application" should {
    ...
    "process undecided items" in {
      ...
      val sink = new DownstreamSinkFunction() {
        override def write(value: tuple.Tuple2[KeyType, ValueType]): Unit = IrishRegistrationDownstreamSpec.result.add(value)
      }
      IrishRegistrationDownstream(metaParameters)
        .processStream(sourceStream, env)(env.getConfig)
        .addSink(sink)

      env.execute()

      IrishRegistrationDownstreamSpec.result.size() must be(6)
    }

  }
}
object IrishRegistrationDownstreamSpec {
