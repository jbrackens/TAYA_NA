...
from gmx_nifi_rest.models import TokenData
...
class OidcTokenScope(OpenIdConnect):
    ...
    async def __call__(self, request: Request) -> TokenData:
        try:
            ...
            raise HTTPException(
                status_code=HTTP_401_UNAUTHORIZED, detail="Wrong token", headers={"WWW-Authenticate": "Bearer"},
            )
    ...
    async def extract_token_data(self, decoded):
        ...
        if not set(self.scopes).issubset(set(perms)):
            ...
            raise HTTPException(
                status_code=HTTP_403_FORBIDDEN, detail="Wrong token",
            )
