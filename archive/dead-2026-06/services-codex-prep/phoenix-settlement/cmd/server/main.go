...
import (
	...
	"github.com/phoenixbot/phoenix-settlement/internal/config"
	"github.com/phoenixbot/phoenix-settlement/internal/handlers"
...
func main() {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Get("/settlement-batches/{batchID}", h.GetSettlementBatch)
		r.Get("/settlement-batches", h.ListSettlementBatches)
		r.Post("/payouts/manual", h.CreateManualPayout)
		r.Post("/reconciliation", h.CreateReconciliation)
		r.Get("/reconciliation/{reconciliationID}", h.GetReconciliation)
	})
	...
	go func() {
		logger.Info("starting phoenix-settlement", slog.Int("port", cfg.Port))
