...
import (
	...
	"github.com/phoenixbot/phoenix-cms/internal/config"
	"github.com/phoenixbot/phoenix-cms/internal/handlers"
...
func main() {
	...
	if cfg.OutboxEnabled {
		...
		worker := outbox.NewWorker(pool, outboxProducer, outbox.Config{Logger: zapLogger.Named("outbox"), PollInterval: pollInterval, BatchSize: cfg.OutboxBatch})
		go worker.Start(ctx)
	}
	...
	go func() {
		logger.Info("starting phoenix-cms", slog.Int("port", cfg.Port))
...
func newZapLogger(environment string) (*gozap.Logger, error) {
	if environment == "development" { return gozap.NewDevelopment() }
