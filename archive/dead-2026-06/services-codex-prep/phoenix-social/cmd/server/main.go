...
import (
	...
	"github.com/phoenixbot/phoenix-social/internal/config"
	"github.com/phoenixbot/phoenix-social/internal/handlers"
...
func main() {
	...
	r.With(middleware.OptionalAuthMiddleware(cfg.JWTSecret, cfg.JWTIssuer, cfg.JWTAudience)).Get("/api/v1/users/{userID}/profile", h.GetProfile)
	r.Get("/api/v1/users/{userID}/followers", h.ListFollowers)

	r.Route("/api/v1", func(r chi.Router) {
		r.Use(middleware.AuthMiddleware(cfg.JWTSecret, cfg.JWTIssuer, cfg.JWTAudience))
		r.Post("/users/{userID}/follow/{targetUserID}", h.FollowUser)
		r.Get("/feed", h.ListFeed)
		r.Post("/messages", h.CreateMessage)
		r.Get("/messages/{conversationID}", h.GetConversation)
	})
	...
	go func() {
		logger.Info("starting phoenix-social", slog.Int("port", cfg.Port))
