...
final class PatronAccountSummaryData(
    ...
    puntersFinder: PuntersFinder)(implicit ec: ExecutionContext, mat: Materializer)
      ...
      case reportingDay: ReportingPeriod.Day =>
        for {
          ...
          reportRows = walletsWithDesignation.map({
            case (wallet, designation) => buildRow(reportingPeriod, wallet, designation, bettingSummaries, openedBets)
          })
  ...
  private def buildRow(
    ...
    PatronAccountSummaryRows.buildReportRow(reportingPeriod, punterWallet, designation, bettingSummary, openedBets)
  }
