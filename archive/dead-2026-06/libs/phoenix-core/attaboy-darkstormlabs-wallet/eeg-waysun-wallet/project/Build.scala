...
object Build {
  ...
  lazy val walletSettings = {
    ...
    val playDevSettings = Seq(PlayKeys.devSettings := Seq("play.server.http.port" -> "15000"))

    mainSettings ++ compilationSettings ++ playDevSettings ++ integrationTestsSettings("it") ++ Seq(
      dockerChmodType := DockerChmodType.UserGroupWriteExecute,
      Docker / packageName := "stella-wallet",
      dockerExposedPorts := {
        val httpServerPort = 15000
