...
import SBTech.Microservices.DataStreaming.DTO.SportBets.v1.{BetTypeEnum, SettlementData, SettlementDataCustomerId, SettlementStatusEnum}
import net.flipsports.gmx.filtering.api.FilteredObjectData
import net.flipsports.gmx.streaming.common.business.{Brand, SourceBrand}
...
class SettlementDataToFilteredObjectData(brandDef: Brand) extends FlatMapFunction[FlinkTuple[SettlementDataCustomerId, SettlementData], FlinkTuple[String, String]] {

  override def flatMap(bet: FlinkTuple[SettlementDataCustomerId, SettlementData], out: Collector[FlinkTuple[String, String]]): Unit = {
    val filteredElements = mapIfShouldBeFiltered(bet.f1)
    filteredElements.foreach(out.collect)
  }

  def mapIfShouldBeFiltered(bet: SettlementData): Seq[FlinkTuple[String, String]] = {
    val brand = brandDef.sourceBrand
    ...
    val betsWithAmountLowerThanMinimum = bets
      ...
      .foldLeft(Seq[(BetInfo, Double)]()) { (result, bet) => {
        result :+ Tuple2(bet, SettlementDataToTopupData.calculateFromBet(bet).asDouble())
      }}
  ...
  def transformRejection(parent: SettlementData, customerId: String, brand: SourceBrand, filteredType: FilteredType) = (bet: BetInfo) =>  {
    ...
    FilteredObjectData.newBuilder()
      ...
      .setCalculatedAmount(SettlementDataToTopupData.calculateFromBet(bet).toString)
      .build()
  }
