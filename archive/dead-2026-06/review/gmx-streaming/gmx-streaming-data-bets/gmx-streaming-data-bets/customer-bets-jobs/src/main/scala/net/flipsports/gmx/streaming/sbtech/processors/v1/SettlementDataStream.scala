...
class SettlementDataStream(metaParameters: MetaParameters, businessMetaParameters: BusinessMetaParameters, configuration: SbTechConfig)
  extends QueuedStream[FlinkTuple[SettlementDataCustomerId, SettlementData], FlinkTuple[Long, CasinoAndSportBetsTopupData]](metaParameters, businessMetaParameters)(SettlementDataStream.inputypeInformation, SettlementDataStream.outputTypeInformation) {

  val kafkaProperties: KafkaProperties = configuration.kafka.withGroupId(s"gmx-streaming.settlement-data-stream-${businessMetaParameters.brand().sourceBrand.name}")
  ...
  override def transform(dataStream: DataStream[FlinkTuple[SettlementDataCustomerId, SettlementData]])(implicit sourceTypeInformation: TypeInformation[FlinkTuple[SettlementDataCustomerId, SettlementData]], targetTypeInformation: TypeInformation[FlinkTuple[Long, CasinoAndSportBetsTopupData]]): DataStream[FlinkTuple[Long, CasinoAndSportBetsTopupData]] =
    dataStream.flatMap(new SettlementDataToTopupData(businessMetaParameters.brand()))

  override def source(implicit ec: ExecutionConfig): SourceFunction[FlinkTuple[SettlementDataCustomerId, SettlementData]] =
    KafkaSource(sourceTopic, kafkaProperties, configuration.sourceTopics.schemaRegistry).specificKeyValue[SettlementDataCustomerId, SettlementData](classOf[SettlementDataCustomerId], classOf[SettlementData])
  ...
  override def filtersDefinition: InputOutputFilter[FlinkTuple[SettlementDataCustomerId, SettlementData], FlinkTuple[Long, CasinoAndSportBetsTopupData]] = new SettlementDataFilter
