...
class RewardsMarketingSettlementDataStreamSpec extends StreamingTestBase
  ...
  "Settlement data stream" must {
    ...
    "publish messages to kafka and stream with filtering" in {
      val messages = SettlementDataProvider
        .apply("rewardsMarketing/settlementdata.json")
        .all
      ...
      withFlink(new InternalFlinkMiniClusterRunner) { _ =>
        withKafka(KafkaProperties()) { (kafkaProperties, schemaRegistryConnectionUrl) =>
          ...
          val job = new RewardsMarketingSettlementDataStream(
