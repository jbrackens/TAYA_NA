...
class FemaleRegistrationBlockExtensionTagMapper(brand: Brand) extends StateChangeTransformer
  ...
  override def transformPayload(): String = FemaleRegistrationBlockExtensionTagMapper.tag.name
...
object FemaleRegistrationBlockExtensionTagMapper {

  val tag: Tag = Tag.FemaleCustomerBlocked

  def apply(brand: Brand): FemaleRegistrationBlockExtensionTagMapper = new FemaleRegistrationBlockExtensionTagMapper(brand)
