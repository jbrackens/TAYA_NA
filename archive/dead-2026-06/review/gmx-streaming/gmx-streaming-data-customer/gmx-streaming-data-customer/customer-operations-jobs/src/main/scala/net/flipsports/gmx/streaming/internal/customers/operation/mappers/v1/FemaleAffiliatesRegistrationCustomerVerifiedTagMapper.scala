...
class FemaleAffiliatesRegistrationCustomerVerifiedTagMapper(brand: Brand) extends StateChangeTransformer
  ...
  override def transformPayload(): String = FemaleAffiliatesRegistrationCustomerVerifiedTagMapper.tag.name
...
object FemaleAffiliatesRegistrationCustomerVerifiedTagMapper {

  val tag: Tag = Tag.CustomerVerified

  def apply(brand: Brand): FemaleAffiliatesRegistrationCustomerVerifiedTagMapper = new FemaleAffiliatesRegistrationCustomerVerifiedTagMapper(brand)
