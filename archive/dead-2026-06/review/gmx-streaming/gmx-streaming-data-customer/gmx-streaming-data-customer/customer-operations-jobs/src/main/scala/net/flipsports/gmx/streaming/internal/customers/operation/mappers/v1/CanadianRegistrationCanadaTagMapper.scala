...
class CanadianRegistrationCanadaTagMapper(brand: Brand) extends StateChangeTransformer
  ...
  override def transformPayload(): String = CanadianRegistrationCanadaTagMapper.tag.name

  override def operationTrigger(): String = OperationTrigger.CustomerRegistration.name
}

object CanadianRegistrationCanadaTagMapper {

  val tag: Tag = Tag.Canada

  def apply(brand: Brand): CanadianRegistrationCanadaTagMapper = new CanadianRegistrationCanadaTagMapper(brand)
}