...
const getExternalPlayerId = (playerIdentifier: PlayerIdentifier) => `${playerIdentifier.brandId}_${playerIdentifier.id}`;
...
const returnGameScript = (url: string, scripts: { realityCheckScript?: string } = {}) => {
  const result = {
    html: `
...
<script>
    ${scripts.realityCheckScript ? `window.brandserver && window.brandserver.addRealityCheckListener(function() { ${scripts.realityCheckScript} });` : ''}
</script>`,
    url,
  };
  return result;
