...
const StyledDateRule = styled.div`
  display: flex;
  align-items: center;
...
const DateRule: React.FC<IProps> = ({ values, setValues, disabled }) => {
  const handleChangeOperator = React.useCallback(
    (newOperator: string | number | string[] | boolean) => {
      ...
      if (newOperator !== "between" && newOperator !== "withinMinutes") {
        return setValues({ ...values, negated: "is", operator: newOperator, values: format(new Date(), "yyyy-MM-dd") });
      }
      setValues({
        ...values,
        operator: newOperator,
