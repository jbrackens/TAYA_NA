...
import type { VieMeterStates } from './vie-progress';
...
class VieJourney implements Journey {
  rewardsCount: number;
  ...
  meterStates: ?VieMeterStates; // TODO: not really needed as there's no meters, just reward balance
  ...
  async init() {
    ...
    if (balance != null) {
      ...
      const rewards = await ldRewards.getRewards(this);
      this.rewardsCount = rewards ? rewards.length : 0;
...
module.exports = { Journey: VieJourney };
