...
const brandDefinition = require('gstech-core/modules/constants').brandDefinitions.FK;
const { parseCommChannelConfig } = require('gstech-core/modules/utils');
const config = require('../common/config');

const CDN = 'https://static.fiksukasino.com/';
...
const productionConfiguration = {
  baseUrl(rest: string = '') {
    return `https://hipspin.com${stripSlashes(rest)}`;
  },
  apiBaseUrl(rest: string = '') {
    return `https://hs-api.luckydino.com${rest}`;
  },
  paymentiq: {
  ...
  clientConfig() {
    return {
      gaAccount: 'G-2PNJSPC9MZ',
      gaDomain: 'hipspin.com',
      liveChat: true,
      gtm: 'GTM-M33VRLG',
...
const developmentConfiguration = {
  ...
  apiUrl() {
    return process.env.API_URL || 'http://localhost:3001/api/FK/v1/';
  },
  clientConfig() {
    return {
...
const commonOptions = {
  liveAgent: {
    ...
    loggedin: (language: string) =>
      ...
      ({ en: 'rjff2b83' })[language] || '8c7ke8cq',
    nonloggedin: (language: string) =>
      // $FlowFixMe[invalid-computed-prop]
      ({ en: 'v7mgnek0', })[language] || 'p84v2vlc',
  },
  paymentFees: {},
  ...
  cms() {
    return config.cmsUrl || 'https://cms.hipspin.com/';
  },
  shortBrandId() {
