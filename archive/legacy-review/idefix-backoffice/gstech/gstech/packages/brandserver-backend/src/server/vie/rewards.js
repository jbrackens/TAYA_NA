...
const getRewards = async (journey: Journey): Promise<ApiReward[]> => {
  ...
  return ledgers.map((rg) => {
    ...
    const locallicationKey = game?.name === 'Delasport' ? 'vierewards.freebets' : `vierewards.${b2.type || ''}spins`;
    const l = localization.localize(journey.req.context.languageISO)(locallicationKey, { count: `<strong>${b2.spins || ''}</strong><b>` }, { format: 'html' }) || '';
