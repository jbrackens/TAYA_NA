...
const { mapReward } = require('./import-tools');
...
const getRewards = async (journey: Journey): Promise<ApiReward[]> => {
  ...
  return ledgers.map((rg) => {
    ...
    const l = localization.localize(journey.req.context.languageISO)(`dinorewards.${b2.type || ''}spins`, { count: `<strong>${b2.spins || ''}</strong><b>` }, { format: 'html' }) || '';
