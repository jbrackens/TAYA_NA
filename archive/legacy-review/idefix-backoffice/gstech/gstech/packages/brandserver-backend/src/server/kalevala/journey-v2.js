...
const { inlineBannerWrapper, mainBanner, campaignTemplate } = require('./banners');
const { getActiveCampaigns } = require('../common/campaign');
const shop = require('./shop-balance');

class KalevalaJourney implements Journey {
  balance: ?Balance;
  ...
  constructor(
    ...
    // $FlowFixMe[method-unbinding]
    (this: any).shopBalance = this.shopBalance.bind(this);
    this.req = req1;
  ...
  shopBalance(): Promise<{ balance: number, progress: number, theme: 'default' | 'double', ... }> {
    return shop.shopBalance(this);
  }
...
module.exports = { Journey: KalevalaJourney };
