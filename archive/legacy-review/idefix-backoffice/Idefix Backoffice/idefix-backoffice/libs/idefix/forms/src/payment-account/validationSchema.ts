import {
  ...
  isCanadianBankAccount,
  isMaskedAccount,
  isBrazilianAccount,
  isPeruvianAccount,
  isChileanAccount,
  isProvidedByDirecta24,
  isValidBrazilBankBranch
} from "../validators";
import { PaymentAccountFormValues, KycProcessPaymentFormValues } from "./types";

type Errors = {
  account?: string;
  bic?: string;
  parameters?: { accountType?: string; bankCode?: string; bankBranch?: string };
  documents?: string;
};

const paymentAccountValidationSchema = (values: PaymentAccountFormValues | KycProcessPaymentFormValues) => {
  const errors: Errors = {};
  ...
  if (isProvidedByDirecta24(method) && (!accountType || accountType.length !== 1)) {
    if (errors.parameters) errors.parameters.accountType = "Should be Savings or Current";
    else errors.parameters = { accountType: "Should be Savings or Current" };
  }

  const bankCode = parameters && parameters.bankCode;
  if (isProvidedByDirecta24(method) && (!bankCode || bankCode.length !== 3)) {
    if (errors.parameters) errors.parameters.bankCode = "Should contain 3 chars";
    else errors.parameters = { bankCode: "Should contain 3 chars" };
  }

  const bankBranch = parameters && parameters.bankBranch;
  if (method === "BrazilBankWD" && !isValidBrazilBankBranch(bankBranch)) {
    if (errors.parameters) errors.parameters.bankBranch = "Invalid bank branch";
    else errors.parameters = { bankBranch: "Invalid bank branch" };
  }
...
export { paymentAccountValidationSchema };
