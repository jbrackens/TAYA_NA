...
export const fetchReport = createAsyncThunk<
  any,
  { reportType: ReportType; values?: { brandId?: string; [key: string]: any }; pageSize?: number; text?: string }
>("reports/fetch-report", async ({ reportType, values, pageSize = 100, text }, { rejectWithValue }) => {
  try {
    const reportValues: any = ["withdrawals", "deposits"].includes(reportType) ? { ...values, pageSize, text } : values;

    const report = (await api.reports.getReport(reportType, reportValues)) as any;
    return report;
  } catch (err) {
