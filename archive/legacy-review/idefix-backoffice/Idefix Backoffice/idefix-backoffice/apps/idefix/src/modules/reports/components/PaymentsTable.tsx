...
const columns = [
  ...
  { label: "Status", name: "status", align: "left", type: "text", style: { minWidth: 58 } },
  {
  ...
  { label: "Amount", name: "amount", align: "left", type: "text", style: { minWidth: 158 } },
...
const PaymentsTable: FC<Props> = ({ items, onFetchMoreData, isLoading, values }) => {
  ...
  const handleFetchAllPayments = useCallback(async () => {
    return await fetchFormattedReport("deposits", { ...values, pageSize: 9999999, text: query });
  }, [query, values]);
      ...
      <Search
        ...
        buttons={
          <DownloadAllCsvButton
            ...
            tooltipText="Download all deposits"
          />
        }
      />
      <Table
