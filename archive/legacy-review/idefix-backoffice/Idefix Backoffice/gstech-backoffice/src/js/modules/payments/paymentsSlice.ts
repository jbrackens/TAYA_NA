...
import { AccountActive, PaymentEvent, PlayerAccount, PlayerPayment } from "app/types";
...
export const fetchPaymentTransactions = createAsyncThunk<
  PlayerPayment[],
  { playerId: number; pageSize?: number; text?: string; sortBy?: string; sortDirection?: SortDirection }
>(
  "payments/fetch-payment-transactions",
  async ({ playerId, pageSize = 100, text, ...rest }, { getState, rejectWithValue }) => {
    ...
    try {
      const transactions = await api.players.getPaymentTransactions(playerId, { status, pageSize, text, ...rest });
      return transactions;
    } catch (err) {
...
export const getPaymentsState = createSelector(getState, payments => payments);
