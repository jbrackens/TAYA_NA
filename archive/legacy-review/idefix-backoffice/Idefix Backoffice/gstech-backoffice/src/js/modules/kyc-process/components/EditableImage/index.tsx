...
import { base64toBlob } from "../../../../core/helpers";
...
export default ({ src, documentId, photoId, onEditImage }: Props) => {
  ...
  const getSizes = useCallback(({ image, percentCrop, param }) => {
    switch (param) {
      case "x":
  ...
  const handleStepSource = useCallback(type => {
  ...
  return sources.length ? (
    <Component
      ...
      handleStepSource={handleStepSource}
      handleCancelEdit={handleCancelEdit}
      handleChangeMagnifyingGlassStatus={handleChangeMagnifyingGlassStatus}
      handleSubmit={handleSubmit}
      crop={crop}
      sourceIndex={sourceIndex}
      sources={sources}
      documentId={documentId}
      magnifyingGlassStatus={magnifyingGlassStatus}
    />
  ) : null;
};
