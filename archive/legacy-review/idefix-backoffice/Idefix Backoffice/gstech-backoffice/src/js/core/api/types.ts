...
export interface PhotosAPI {
  uploadPhoto: (photoDraft: any, photoName?: string) => Promise<{ id: string; originalName: string }>;
  removePhoto: (photoId: number) => Promise<OKResponse>;
  getDocument: (documentId: string) => Promise<any>;
}

export interface ReportsAPI {
  getReport: (type: ReportType, values: { brandId?: string; [key: string]: any }) => Promise<any>;
}
...
export interface SettingsAPI {
  ...
  getGameManufacturer: (gameManufacturerId: string) => Promise<GameManufacturer & { blockedCountries: any[] }>;
  ...
  updatePaymentProviderDetails: (paymentProviderId: number, values: any) => Promise<any>;
...
export interface UsersAPI {
  ...
  update: (userId: number, userDraft: UserDraft) => Promise<FullUser>;
...
export interface PlayersAPI {
  ...
  getTicket: (externalRoundId: string) => Promise<Ticket>;
  ...
  getLedgers: (playerId: number, params: any) => Promise<any>;

  markRewardUsed: (playerId: number, groupId: number, values?: { comment: string }) => Promise<any>;

  getRewards: (brandId: string, groupId: string) => Promise<{ data: Reward[] }>;

  addReward: (rewardId: number, values: { playerId: number; count: number; comment?: string }) => Promise<any>;

  getProgresses: (brandId: string, playerId: number) => Promise<{ data: { progresses: RewardProgress[] } }>;

  getBalances: (brandId: string, playerId: number) => Promise<{ data: any }>;
