...
class AchievementApplicationLoader extends ApplicationLoader {
  override def load(context: Context): Application = {
    new AchievementComponents(context).application
  }
}

class AchievementComponents(context: Context)
    extends BuiltInComponentsFromContext(context)
    with AchievementModule
