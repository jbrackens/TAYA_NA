...
import stella.achievement.SampleObjectFactory.defaultHeaders
import stella.achievement.SampleObjectFactory.errorOutputResponse
import stella.achievement.SampleObjectFactory.testAuthContext
...
trait RoutesWithMockedBoundedContextSpecBase
  ...
  protected val boundedContext: AchievementBoundedContext = mock[AchievementBoundedContextImpl]
  ...
  override def fakeApplication(): Application = new TestAchievementAppBuilder {
    override def createAchievementComponents(context: Context): AchievementComponents =
      new AchievementComponents(context: Context) {
        ...
        override lazy val achievementBoundedContext: AchievementBoundedContext = boundedContext
      }
  }.build()
