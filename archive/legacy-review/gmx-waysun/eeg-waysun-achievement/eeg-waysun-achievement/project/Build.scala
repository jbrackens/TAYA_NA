...
object Build {
  ...
  lazy val achievementSettings = commonSettings ++ integrationTestsSettings ++
    Seq(
      dockerChmodType := DockerChmodType.UserGroupWriteExecute,
      Docker / packageName := "stella-achievement",
      Compile / publishArtifact := true,
      dockerExposedPorts := Seq(12000))
  ...
  lazy val commonSettings = {
    ...
    val playDevSettings = Seq(PlayKeys.devSettings := Seq("play.server.http.port" -> "12000"))
