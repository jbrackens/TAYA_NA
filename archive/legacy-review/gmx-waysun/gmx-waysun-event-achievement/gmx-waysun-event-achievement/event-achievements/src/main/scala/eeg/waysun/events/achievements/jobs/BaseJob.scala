...
import eeg.waysun.events.achievements.configs.AppConfigDef.{AppConfig, ConfigurationLoader}
...
class BaseJob(jobName: String, params: BusinessMetaParameters) extends DefaultExecutionEnvironment {
  ...
  lazy val name = s"Achievements $jobName events on brand ${businessMetaParams.brand().sourceBrand.name}"
