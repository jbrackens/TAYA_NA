...
TOPICS=(
   "stella-messaging.default-achievement-definition >> --replication-factor 3 --partitions 3 --config min.compaction.lag.ms=10000  --config cleanup.policy=compact --config delete.retention.ms=10000  --config segment.ms=10000 --config segment.bytes=104857600 --config min.cleanable.dirty.ratio=0.1"
   "stella-streaming.default-achievement-achieved            >> --partitions 6 --replication-factor 3 --config retention.ms=3000000000"
)
