...
class SECRET_BOX:  # noqa
    ...
    def encrypt(self, obj: Any) -> str:
        data = (obj, int(datetime.utcnow().timestamp()))
    ...
    def decrypt(self, cipher: str, max_age: int = None) -> Any:
        ...
        if max_age:
            assert max_age > 0
            if sign_date + max_age < current_timestamp:
                raise CryptoError(f"Data expired over {current_timestamp-max_age-sign_date} seconds.")
        return obj
