...
object AppConfigDef {
  ...
  case class SourceTopics(definition: String, raw: String, schemaRegistry: String)

  case class TargetTopics(validated: String, failed: String, schemaRegistry: String)

  object TargetTopics {
    def apply(config: Config): TargetTopics = {
      TargetTopics(
        validated = config.getString("validated"),
        failed = config.getString("failed"),
