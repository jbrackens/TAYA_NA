variable "rules" {
  ...
  default = {
    # AsparaDB for RDS
    apsaradb-tcp = [1921, 1921, "tcp", "ApsaraDB RDS"]
    # Carbon relay
    ...
    # EMR
    emr-kafka-schema-registry-tcp = [8081, 8081, "tcp", "EMR Kafka Schema Registry"]
    emr-kafka-rest-proxy-tcp      = [8082, 8082, "tcp", "EMR Kafka Rest Proxy"]
    emr-kafka-connect-tcp         = [8083, 8083, "tcp", "EMR Kafka Connect"]
    emr-kafka-metadata-tcp        = [8084, 8084, "tcp", "EMR Kafka Metadata"]
    emr-kafka-manager-tcp         = [8085, 8085, "tcp", "EMR Kafka Manager"]
    # HTTP
...
variable "auto_groups" {
  ...
  default = {
    ...
    emr = {
      ingress_rules     = ["emr-kafka-schema-registry-tcp", "emr-kafka-rest-proxy-tcp", "emr-kafka-connect-tcp", "emr-kafka-metadata-tcp", "emr-kafka-manager-tcp"]
      ingress_with_self = ["all-all"]
      egress_rules      = ["all-all"]
    }
    http-80 = {
