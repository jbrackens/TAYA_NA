...
variable "security_group_id" {
  description = "Security group ID to pass, otherwise default will be created"
  type        = string
  default     = null
}
variable "slb_internet_enabled" {
  description = "Should k8s API be accessable from Internet"
  type        = bool
  default     = true
}
variable "worker_payment_type" {
  description = "Workers charge type components in kubernetes cluster"
  type = object({
