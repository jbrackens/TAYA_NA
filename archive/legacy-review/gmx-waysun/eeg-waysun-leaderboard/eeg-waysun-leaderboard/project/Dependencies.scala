...
object Dependencies {
  ...
  private val stellaCommonKafkaDeps = Seq(
    "stella"                           %% "common-kafka"                    % Versions.stellaCommon
  )

  private val stellaCommonPlayTestUtilsDeps = Seq(
    "stella"                           %% "common-play-test-utils"          % Versions.stellaCommon
  )

  private val stellaDataApiDeps = Seq(
    "stella"                           %% "aggregation-results"             % Versions.stellaDataApi
  )
  ...
  private val redisCacheDeps = Seq(
    PlayImport.cacheApi,
    "com.github.karelcemus"            %% "play-redis"                      % Versions.playRedis,
    "io.altoo"                         %% "akka-kryo-serialization"         % Versions.akkaKryoSerialization
  )

  private val tapirModelDeps = Seq(
  ...
  private val testingDepsPlay = Seq(
    "org.scalatestplus.play"           %% "scalatestplus-play"              % Versions.scalaTestPlay
  ).map(_ % Test)
  // @formatter:on

  val integrationTestCommonModuleDeps =
    integrationTestsDeps

  val commonModuleDeps =
    catsDeps ++
    dbDeps ++
    enumeratumDeps ++
    jacksonDeps ++
  ...
  val serverDeps =
    integrationTestsDeps.map(_ % Test) ++
    macWireDeps ++
    redisCacheDeps ++
    tapirDeps ++
    tapirPlayDeps ++
    testingDeps ++
    testingDepsPlay ++
    stellaCommonKafkaDeps ++
    stellaCommonPlayTestUtilsDeps
