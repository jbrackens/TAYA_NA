...
import stella.leaderboard.ingestor.it.containers.kafka.KafkaProperties.schemaRegistryProperty
...
case class KafkaProperties(properties: Properties = new Properties) extends Serializable {
  ...
  def withOffsetResetConfig(offsetRestConfig: String): KafkaProperties =
    if (isNotNullAndNotBlank(offsetRestConfig)) {
      val props = new Properties()
  ...
  private def isNotNullAndNotBlank(offsetRestConfig: String): Boolean =
    Option(offsetRestConfig).exists(!_.isBlank)
}
