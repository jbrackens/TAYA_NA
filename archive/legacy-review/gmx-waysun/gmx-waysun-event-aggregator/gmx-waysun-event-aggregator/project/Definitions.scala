...
object Deps {
  ...
  private val flinkMetrics = Seq("org.apache.flink" % "flink-metrics-dropwizard" % Versions.flink.version)
  ...
  private val scalaTests = Seq(
    ...
    "com.dimafeng" %% "testcontainers-scala-scalatest" % Versions.scalaTests.testContainers % Test,
  ...
  private val externalApi = Seq(
    "stella" %% "platform-events" % Versions.externalApi.waysunDataApi,
    "stella" %% "validators" % Versions.externalApi.waysunDataApi,
    "stella" %% "event-configurations" % Versions.externalApi.waysunDataApi,
    "stella" %% "aggregation-results" % Versions.externalApi.waysunDataApi,
    "stella" %% "aggregation-rule-configurations" % Versions.externalApi.waysunDataApi)

  private val json = Seq("com.fasterxml.jackson.core" % "jackson-databind" % Versions.jackson.version % Test)

  private val kafka = Seq("io.confluent" % "kafka-avro-serializer" % Versions.kafka.version)
...
object Versions {
  ...
  object internalApi {
    ...
    val streamingEngine = "0.0.49"

  }

  object externalApi {

    val waysunDataApi = "0.0.35"
  }
