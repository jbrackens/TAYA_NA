...
locals {
  ...
  security_group_name        = "${local.project_name}-${local.environment}-redis-sg"
  security_group_description = "outgoing traffic rds"
}

dependency "outgoing-redis" {
  config_path = "../outgoing-redis"
...
inputs = {
  ...
  ingress_with_source_sgids = [
    {
      source_sgid = dependency.outgoing-redis.outputs.security_group_id
      port        = "6379"
      description = "allow outgoing redis"
    }]
