...
function k8s.enable_hpa() {
  local hpa_name="${1}"
  local min_replicas="${2:-${CFG_REPLICAS:-''}}"
  local max_replicas="${3:-${CFG_MAX_REPLICAS:-''}}"
  local cpu_utilization="${4:-${CFG_HPA_CPU_UTILIZATION:-''}}"

  if [[ -z "${hpa_name}" || -z "${min_replicas}"  || -z "${max_replicas}" || -z "${cpu_utilization}" ]]; then
    log.error "Wrong parameters for 'k8s.enable_hpa()'."
    log.error " hpa_name = '${hpa_name}'"
    log.error " min_replicas = '${min_replicas}'"
    log.error " max_replicas = '${max_replicas}'"
    log.error " cpu_utilization = '${cpu_utilization}'"
    exit "${ERR_WRONG_CFG_PARAMS_CONFIGURATION}"
  fi

  local tpl=
  tpl=$(common.get_tmp_file) || die
  # log.info "About  files to create one YAML:"
...
function k8s.create_namespace_if_not_exists() {
  ...
  if [ "${__K8S_NAMESPACES}" == "" ] ; then
    ...
    __K8S_NAMESPACES="$(kubectl get namespaces | grep Active | cut -d' ' -f1)" || __K8S_NAMESPACES="$(kubectl get namespaces | grep Active | cut -d' ' -f1)" || die
    log.info "Done"
  fi
