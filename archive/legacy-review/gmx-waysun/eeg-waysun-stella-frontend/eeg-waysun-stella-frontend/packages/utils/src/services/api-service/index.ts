...
export const useApiHook = (
  ...
  const refreshData = refreshCall?.data as RefreshData;
  ...
  useEffect(() => {
    if (isAuthorizationRequired) {
      if (isRefreshTokenUpToDate(getRefreshTokenExpDate())) {
        ...
        if (
          response?.data?.details?.errorCodes.some(
  ...
  useEffect(() => {
    if (
      !refreshCallError &&
      refreshData?.details?.id_token &&
      refreshData?.details?.refresh_token &&
      refreshData?.details?.expires_in
    ) {
      ...
      saveTokenExpDate(
        appendSecondsToTimestamp(refreshData.details.expires_in, now),
        appendSecondsToTimestamp(refreshData.details.refresh_expires_in, now),
      );
      setStateRef({
        ...stateRef.current,
        isReadyToTrigger: stateRef.current.forceRefresh ? false : true,
  ...
  useEffect(() => {
    const completeExecution = () => {
      ...
      if (shouldSucceed && isReadyToReturn) {
        setStateRef({
          ...stateRef.current,
  ...
  const getDataToReturn = (
    ...
    if (isAuthorizationRequired && !hasInvalidAuthTokenError) {
      ...
      return {
        error: hasError ? transformedError : undefined,
        data: hasError ? undefined : apiResponse.data,
        isLoading,
