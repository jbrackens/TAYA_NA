...
import phoenix.oddin.data.FixtureChange

class FixtureChangeConsoleOutput extends AkkaStreamlet {

  val inlet = AvroInlet[FixtureChange]("in")
  ...
  override def createLogic =
    new RunnableGraphStreamletLogic() {
      ...
      val flow = FlowWithCommittableContext[FixtureChange].map { data =>
        log.info(s"$data")
        data
      }
    }
