    ...
    s"${testSetup.identifier}" should {
      ...
      "store and return audit log entries" in {
        ...
        val expectedLogEntries = List(
          ...
          AccountCreationEntry(Api.generatePunterId(), now.plusSeconds(6)),
          AccountClosureEntry(Api.generatePunterId(), now.plusSeconds(7)))
        ...
        val expectedResult = PaginatedResult(expectedLogEntries.reverse.slice(0, 4), 8, pagination)
