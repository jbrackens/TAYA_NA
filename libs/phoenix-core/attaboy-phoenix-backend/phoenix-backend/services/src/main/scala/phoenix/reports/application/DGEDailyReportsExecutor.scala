...
private[reports] final class DGEDailyReportsExecutor(
  ...
  val dailyReports = List(
    ...
    new PatronAccountSummary(
      new PatronAccountSummaryData(betEventsRepository, openedBetsFinder, walletSummaries, puntersFinder)),
    new PendingTransactionDepositsWithdrawals(
      new PendingTransactionDepositsWithdrawalsData(transactionFinder, puntersFinder)),
    new WageringSummary(new WageringSummarySportsData(betEventsRepository, fixtureMarketFinder)),
    new Cancelled(new CancelledData(betEventsRepository, betsFinder, fixtureMarketFinder, puntersFinder)),
    new Resettle(new ResettleData(betEventsRepository, betsFinder, fixtureMarketFinder, puntersFinder)),
    new ResultDetails(new ResultDetailsData(betEventsRepository, fixtureMarketFinder, puntersFinder)),
    new ResultSummary(new ResultSummaryData(betEventsRepository)),
    new SportsLiability(new SportsLiabilityData(openedBetsFinder, fixtureMarketFinder, puntersFinder)),
    new Voids(new VoidsData(betEventsRepository, betsFinder, puntersFinder, fixtureMarketFinder)))
