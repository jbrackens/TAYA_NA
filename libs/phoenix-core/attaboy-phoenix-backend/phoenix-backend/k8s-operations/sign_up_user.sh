...
if [[ $# -ne 9 ]]; then
  echo "Usage: $0 <keycloak_realm> <keycloak_url> <username> <email> <first-name> <last-name> <balance> <phone-number> <admin=true|false>"
  exit 1
fi
...
if [[ $admin = true ]]; then
  "$script_dir"/grant_admin_to_keycloak_user.sh "$keycloak_realm" "$keycloak_url" "$username"
fi
