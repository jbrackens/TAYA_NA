...
3. ~~**#6 Hydration suppression (`pages/_app.js`)**~~ ✅ **DONE 2026-05-21 (uncommitted — working tree).** Made `useTimezone` (`packages/utils/src/hooks/timezone/index.ts`) mounted-aware via the established `SidebarMenu` pattern: `useState(false)` + `useEffect(()=>setMounted(true),[])`, and read the saved TZ only `mounted && typeof localStorage !== "undefined" ? getTimezone() : undefined`. The first render (server + first client render) now falls back to `dayjs.tz.guess()` on both sides — identical on the same machine — so dated cells hydrate without a mismatch; after mount the hook switches to the saved/browser TZ. Rebuilt `@phoenix-ui/utils` dist (it resolves via `main: dist/index.js`; `transpilePackages` doesn't change the entry point). The **entire** DEV-ONLY block in `_app.js` (both the `console.error` filter and the `NEXTJS-PORTAL` `MutationObserver`) is deleted. **Verified in-browser** (office dev :3001, admin login) with a divergent saved TZ (`Asia/Tokyo` vs machine `Europe/Malta`): every reachable Pages-Router page — `/account/settings`, `/logs`, `/account/security`, `/terms-and-conditions` — renders with the Next dev badge `data-error="false"` and zero hydration phrases in the console. Note: the `/risk-management/*` Pages-Router subtree redirects to `/dashboard` (next.config), and the admin tables fetch data client-side (no SSR date cells), so the practical SSR `useTimezone` surface was already small. The `/account/settings` timezone label keeps its own per-element `suppressHydrationWarning` — it reads `getTimezone()` directly (not via `useTimezone`), so it's independent of this fix.
...
5. ~~**#12 Warning-debt cleanup in app/**~~ ✅ **DONE 2026-05-21 (uncommitted) — subsumed by #6.** The `console.error` suppression sledgehammer in `pages/_app.js` was deleted wholesale alongside the `MutationObserver` when #6 made `useTimezone` hydration-safe. No targeted React-level filter was needed — with the root cause fixed there are no benign hydration warnings left to ignore (verified across the reachable Pages-Router pages).
