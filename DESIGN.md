...
| 2026-04-28 | **P8 implementation shipped (PR p8-light-pivot)** | Remote-agent run (Claude Sonnet 4.6) on branch `p8-light-pivot`, 8 commits. PRs #1–#5 (P7 secondary surfaces + token cleanup + legacy carcass removal) all confirmed merged before work began. Amended spec values used throughout: `--yes-text #1A6849` (6.7:1 on white), `--no-text #A8472D` (5.8:1 on white), TopBar uses opaque cream — no `backdrop-filter`, focus ring `#0E7A53` (4.8:1 on cream). Files touched: `app/globals.css` (token swap + body::before removal + chart-paper grid + focus ring rule), `app/components/prediction/MarketCard.tsx` (full P8 rewrite), `app/components/prediction/utils/marketImage.ts` (new — deterministic monogram fallback), `app/components/prediction/TopBar.tsx` (hover/menu/shadow/divider on light), `app/components/prediction/MarketChart.tsx` (line strokes → yes-text/no-text, grid lines + labels on light, mc-switcher bg), `app/predict/page.tsx` (hero chart strokes + rh-change text on light), `app/components/prediction/PredictFooter.tsx` (stale dark hex → tokens). Legacy dark tokens (`--glass-*`, `--rim-*`, `--chroma-*`, `--bg-{navy,teal,mint,azure}`, remaining `--accent-{lo,gradient}`) left in `globals.css` per per-surface retirement policy. Visual verification deferred to human review (no browser in remote sandbox). `npx tsc --noEmit` run — see PR body for status. Deferred: secondary non-prediction surfaces with stale hardcoded hex (QuickStake, SportSidebar, IdComplyModal, LoginForm, GeoComplyCheck, error pages) — their dark card backgrounds keep the light text readable; full retirement on per-surface cleanup PRs. |
