...
BACKOFFICE_DIR="${ROOT_DIR}/frontend/packages/office"
...
build_workspace_packages() {
  ...
  for pkg in utils api-client design-system; do
    local pkg_dir="${ROOT_DIR}/frontend/packages/${pkg}"
    if [ ! -f "${pkg_dir}/dist/index.js" ]; then
      log_info "  Building frontend/${pkg}..."
      cd "${pkg_dir}"
...
cleanup_router_conflicts() {
  ...
  # frontend conflicts
  local bo_pages="$ROOT_DIR/frontend/packages/office/pages"
