...
import type { PunterRoles } from "@taptrade-ui/utils/dist/types/punter";
import {
  validateSession,
  buildRedirectUrl,
  clientNukeAuth,
    ...
    if (!isEqual(values, prevValues)) {
      const triggerCheck = async () => {
        try {
          ...
          // Never leave the spinner stuck — always clear on error
          setVerificationInProgress(false);
        }
      };
      if (!isAuthRoute) {
        // In dev mode, don't block rendering with a spinner during navigation.
        // The mock server doesn't support full session validation, so this
        // would cause the page to appear frozen on every route change.
        if (process.env.NODE_ENV !== "development") {
