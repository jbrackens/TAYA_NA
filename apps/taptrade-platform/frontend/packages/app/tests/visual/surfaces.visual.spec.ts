...
test.describe("anonymous surfaces @anon", () => {
  ...
  test("discover (/predict)", async ({ page }) => {
    await prep(page, "/predict");
    await shoot(page, "discover");
  });

  test("market page", async ({ page }) => {
    await prep(page, `/market/${MARKET_TICKER}`);
  ...
  test("login (/auth/login)", async ({ page }) => {
    await prep(page, "/auth/login/");
    await shoot(page, "login");
  });
});

test.describe("authenticated surfaces @authed", () => {
  test("market page with trade ticket", async ({ page }) => {
    await prep(page, `/market/${MARKET_TICKER}`);
    await shoot(page, "market-authed");
  });

  test("store (/store)", async ({ page }) => {
    await prep(page, "/store");
    await shoot(page, "store");
  });

  test("portfolio", async ({ page }) => {
    await prep(page, "/portfolio");
