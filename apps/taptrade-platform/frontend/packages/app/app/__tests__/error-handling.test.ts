...
describe("Error Handling", () => {
  describe("parseApiError", () => {
    ...
    it("marks 409 conflict as retryable", () => {
      const body = JSON.stringify({
        error: { code: "conflict", message: "serialization failure" },
      });
  ...
  describe("calculateBackoff", () => {
    it("increases exponentially", () => {
      ...
      assert.ok(
        delay1 > delay0,
        `delay1 (${delay1}) should be > delay0 (${delay0})`,
      );
      assert.ok(
        delay2 > delay1,
        `delay2 (${delay2}) should be > delay1 (${delay1})`,
      );
    });
    ...
    it("caps at maxMs", () => {
      ...
      assert.ok(
        delay <= 6250,
        `delay (${delay}) should be <= 6250 (5000 + 25% jitter)`,
      );
    });
