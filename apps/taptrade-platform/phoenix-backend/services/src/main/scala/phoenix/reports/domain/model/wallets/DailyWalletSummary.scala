...
import phoenix.wallets.WalletsBoundedContextProtocol.TransactionReason.FundsReservedForPrediction
import phoenix.wallets.WalletsBoundedContextProtocol.TransactionReason.PredictionCancelled
import phoenix.wallets.WalletsBoundedContextProtocol.TransactionReason.PredictionLost
import phoenix.wallets.WalletsBoundedContextProtocol.TransactionReason.PredictionPushed
import phoenix.wallets.WalletsBoundedContextProtocol.TransactionReason.PredictionResettled
import phoenix.wallets.WalletsBoundedContextProtocol.TransactionReason.PredictionVoided
import phoenix.wallets.WalletsBoundedContextProtocol.TransactionReason.PredictionWon
  ...
  private def updateTurnover(transaction: Transaction): Turnover =
    transaction match {
      case BetTransaction(_, reason, _, _, bet, _) =>
        reason match {
          case FundsReservedForBet | FundsReservedForPrediction =>
            turnover.plus(bet.stake.value.moneyAmount)
          case BetCancelled | PredictionCancelled =>
            turnover.subtract(bet.stake.value.moneyAmount)
          case BetWon | BetLost | BetVoided | BetPushed | BetResettled |
              PredictionWon | PredictionLost | PredictionVoided | PredictionPushed | PredictionResettled =>
            turnover
        }
