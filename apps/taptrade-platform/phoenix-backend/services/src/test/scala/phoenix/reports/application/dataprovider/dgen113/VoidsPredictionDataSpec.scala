...
final class VoidsPredictionDataSpec extends AnyWordSpecLike with Matchers with FutureSupport {

  "A VoidsPredictionData" should {
    "produce void rows for voided prediction orders" in {
      ...
      val punterId = PunterId(s"prediction-voided-${System.nanoTime()}")
      ...
      val preparedOrder = await(
        readModels.prepareOrder(
          ...
          PredictionPlaceOrderRequest(
            ...
            outcomeId = "yes",
            stakeUsd = BigDecimal(9))))
        .toOption
        .get
      val storedOrder = await(readModels.placePreparedOrder(preparedOrder, ReservationId(s"prediction-voided-${System.nanoTime()}")))
      await(readModels.settleStoredOrder(storedOrder.orderId, "voided", Some("Market invalidated"), Some("talon-admin")))

      val rows = await(
        new VoidsPredictionData(readModels, new PuntersFinder(puntersRepository)).getData(reportingPeriod))
        .filter(_.patronId == PatronIdField(punterId))

      rows should have size 1
      val row = rows.head
      row.copy(transactionTime = TimeField(row.transactionTime.value)) shouldBe PredictionVoidsRow(
        gamingDate = DateField(reportingPeriod.periodStart),
        transactionTime = TimeField(row.transactionTime.value),
        ...
        issuanceDateTime = DateTimeField(OffsetDateTime.parse(storedOrder.createdAt)),
        ...
        stakeAmount = MoneyField(BigDecimal(9)),
        employeeNameSystemIdentifier = StringField("talon-admin"),
        reasonForVoid = StringField("Market invalidated"))
    }
  }
}
