...
- **v4.4:** frontend build remediation **completed** (2026-05-17). All **6** pre-existing build defects fixed (the v4.3 four plus #5 Next-16-Turbopack-vs-`--webpack`, #6 undeclared `@phoenix-ui/api-client` dep). Both `app` and `office` images now build (`BUILD_EXIT=0`). **Path A validated end-to-end through real Caddy + the real built image**: single origin `localhost:8080` serves the Next page, proxies `/api` to gateway, login sets a host-only `SameSite=Lax` cookie, authed REST returns u-1 demo data, and `ws://…/ws` upgrades `101` on that cookie. The v4.3 "needs a build-remediation pass before any demo date" prerequisite is now **done**. Remaining: commit the A0 fixes, optionally multi-stage the 7.6 GB image, then deploy. Full evidence + final corrected A0 in **Validation status**.
...
- **Backend (Blocker 1 + demo data):** running gateway has `WALLET_STORE_MODE=db`/`GATEWAY_READ_REPO_MODE=db`, auth `AUTH_STORE_MODE=db`; migrations goose v20; live DB confirms `u-1 | demo@phoenix.local | player` with the rich seeded portfolio (`accuracyPct:70`, 10 predictions, $306.91). `make demo-data` is mandatory.
...
|---|---|---|---|
| 1 | `.dockerignore` excludes `/packages/utils` (hard `@phoenix-ui/*` workspace dep) | packaging | un-excluded in `.dockerignore` |
...
| 3 | `@phoenix-ui/utils` `tsc` fails — `TS7016` no types for `qs` | TS-debt | ambient `qs` shim (`packages/utils/src/qs-shim.d.ts`); proper fix = add `@types/qs` + refresh lockfile |
| 4 | `@phoenix-ui/app` `build` runs `bootstrap:locales` → `ENOENT` on `packages/app/translations/` — source deleted in commit #15 "bulk-delete legacy package-root carcass"; generated `public/static/locales` (130 files) is now the **committed canonical** artifact | half-finished i18n migration | drop vestigial `bootstrap:locales` from app `build`; un-`.dockerignore` `public/static/locales` so the canonical locales ship |
...
| 6 | `@phoenix-ui/api-client` imported by app + in `transpilePackages` but **absent from app `package.json` dependencies** → `--include-dependencies` skips it → no `dist/index.d.ts` → TS `Cannot find module '@phoenix-ui/api-client'` | undeclared dependency | declare `@phoenix-ui/api-client` in app `dependencies` (lockfile-safe — workspace deps aren't in yarn.lock) |

### Final corrected A0 (supersedes the A0 in PATH A below — this is the validated recipe)

`FROM node:20-bookworm-slim`; drop `--mount=secret,id=npmrc` + the `! test -e` line; `.npmrc` `@phoenix-ui`→npmjs; **un-exclude `/packages/utils` AND `public/static/locales`** in `.dockerignore`; replace `RUN yarn bootstrap` + `RUN yarn build …` with **`RUN yarn lerna run build --scope @phoenix-ui/$module_name --include-dependencies`**; `packages/app` `build` = `next build --webpack && ./scripts/check-no-upstream-leak.sh` (drop `bootstrap:locales`); add `@phoenix-ui/api-client` to `packages/app` dependencies; add `qs` typing to `packages/utils` (shim now; `@types/qs` properly). Both `module_name=app` and `office` build clean with this.
...
`talon-backoffice/docker/frontend.Dockerfile` (multi-stage standalone — THE validated build), `talon-backoffice/.npmrc`, `talon-backoffice/.dockerignore`, `talon-backoffice/packages/app/package.json`, `talon-backoffice/packages/{app,office}/next.config.js` (`output:"standalone"`), `talon-backoffice/packages/utils/src/qs-shim.d.ts`, `docker-compose.demo.yml` (prod overlay), `Caddyfile` (prod, 2 single-origin hosts + auto-TLS), `Caddyfile.local` (local validation). Commit `b60c0d01`, `--no-verify` (infra-only, validated by build+run, not covered by the `yarn test` unit gate — user-authorized).
...
| `demo@phoenix.local` → user **`u-1`**; `make demo-data` populates `u-1` (12 positions, $5k, 70% acc.) | `auth/handlers.go:152`; `seed/demo_phase4_user.go:11,107`; `wallet_topup.go:35` |
| Frontend: one image, two roles via `module_name`/`MODULE_NAME` → `yarn start:app` / `start:office` | `talon-backoffice/docker/frontend.Dockerfile:26-37`; root `package.json` scripts |
...
`talon-backoffice/docker/frontend.Dockerfile`:
...
- `talon-backoffice/.npmrc`: point `@phoenix-ui` at `https://registry.npmjs.org/` or delete the line (deps are workspace-resolved)

Build both roles once:
```bash
cd Taya_Na_Predict/apps/Phoenix-Predict-Combined/talon-backoffice
...
# demo@phoenix.local IS u-1; do NOT set AUTH_DEMO_USER_ID or "clean up" seeds.
...
curl -i -c /tmp/cj -X POST https://demo.99rtp.io/api/v1/auth/login \
  -H 'content-type: application/json' -d '{"username":"demo@phoenix.local","password":"demo123"}'
...
2. **`talon-backoffice/` is canonical** (`phoenix-frontend/` is a stale dup). Build from `talon-backoffice/`.
