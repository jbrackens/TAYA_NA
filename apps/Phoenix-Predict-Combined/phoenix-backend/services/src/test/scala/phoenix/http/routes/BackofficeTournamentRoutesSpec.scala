...
import phoenix.support.DataGenerator.generateTournamentId
import phoenix.support.FutureSupport
import phoenix.time.FakeHardcodedClock
import phoenix.wallets.infrastructure.http.WalletRoutes

final class BackofficeTournamentRoutesSpec extends RoutesSpecSupport with FutureSupport {
  ...
  private val ThePunterId = PunterId("69790b82-6c65-4ed9-91eb-b4ba0b8542a8")
  private val inMemoryMarkets: InMemoryMarkets = new InMemoryMarkets()
  private val marketsBoundedContext =
    MarketBoundedContextMock.apply(
      tournamentDisplayFn = tournamentId => inMemoryMarkets.makeTournamentDisplayable(tournamentId),
      tournamentNotDisplayFn = tournamentId => inMemoryMarkets.makeTournamentNotDisplayable(tournamentId))

  private val routes = {
    val wallets = new WalletContextProviderSuccess(clock)
    val punters = new PuntersContextProviderSuccess()
    val bets = BetsBoundedContextMock.betsSearchMock(PaginatedResult.apply(Seq(), 0, 0, 0, false))
    ...
    val markets = marketsBoundedContext
  ...
  "Backoffice tournament routes" when {
    "POST /admin/trading/tournaments/:tournamentId/make-displayable" should {
      ...
      assertAdminRoleRequired(Post(s"/admin/trading/tournaments/${tournamentId.value}/make-displayable"))

      "return 204 NoContent" in {
        withAdminToken(
          Post(s"/admin/trading/tournaments/${tournamentId.value}/make-displayable")) ~> underTest ~> check {
          status shouldEqual StatusCodes.NoContent
          inMemoryMarkets.displayableTournaments should contain(tournamentId)
        }
      }
    }

    "POST /admin/trading/tournaments/:tournamentId/make-not-displayable" should {
      val tournamentId = generateTournamentId()
      await(inMemoryMarkets.makeTournamentDisplayable(tournamentId))
      assertAdminRoleRequired(Post(s"/admin/trading/tournaments/${tournamentId.value}/make-not-displayable"))

      "return 204 NoContent" in {
        withAdminToken(
          Post(s"/admin/trading/tournaments/${tournamentId.value}/make-not-displayable")) ~> underTest ~> check {
          status shouldEqual StatusCodes.NoContent
          inMemoryMarkets.displayableTournaments should not contain tournamentId
        }
      }
    }
  }
