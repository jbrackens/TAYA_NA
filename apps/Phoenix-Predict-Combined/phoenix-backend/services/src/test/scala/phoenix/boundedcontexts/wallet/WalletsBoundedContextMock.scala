...
class WalletContextProviderSuccess(clock: Clock) extends WalletsBoundedContext {
  ...
  override def finalizePrediction(walletId: WalletId, reservationId: ReservationId, outcome: BetPlacementOutcome)(implicit
      ec: ExecutionContext): EitherT[Future, BetFinalizationError, Balance] =
    finalizeBet(walletId, reservationId, outcome)
  ...
  override def refinalizePrediction(walletId: WalletId, bet: Bet, newOutcome: BetPlacementOutcome)(implicit
      ec: ExecutionContext): EitherT[Future, BetFinalizationError, Balance] =
    refinalizeBet(walletId, bet, newOutcome)
