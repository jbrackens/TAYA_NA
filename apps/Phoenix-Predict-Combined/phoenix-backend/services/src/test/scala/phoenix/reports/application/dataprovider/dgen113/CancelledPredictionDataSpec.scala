...
final class CancelledPredictionDataSpec extends AnyWordSpecLike with Matchers with FutureSupport {

  "A CancelledPredictionData" should {
    "produce cancellation rows for cancelled prediction orders" in {
      ...
      val punterId = PunterId(s"prediction-cancelled-${System.nanoTime()}")
      ...
      val preparedOrder = await(
        ...
        .toOption
        .get
      val storedOrder = await(readModels.placePreparedOrder(preparedOrder, ReservationId(s"prediction-cancelled-${System.nanoTime()}")))
      await(readModels.cancelStoredOrder(storedOrder.orderId, Some("Trader requested rollback")))

      val rows = await(
        new CancelledPredictionData(readModels, new PuntersFinder(puntersRepository)).getData(reportingPeriod))
        .filter(_.patronId == PatronIdField(punterId))

      rows should have size 1
      val row = rows.head
      row.copy(
        transactionTime = TimeField(row.transactionTime.value)) shouldBe PredictionCancelledRow(
        ...
        stakeAmount = MoneyField(BigDecimal(15)),
        reasonForCancellation = StringField("Trader requested rollback"))
    }
  }
}
