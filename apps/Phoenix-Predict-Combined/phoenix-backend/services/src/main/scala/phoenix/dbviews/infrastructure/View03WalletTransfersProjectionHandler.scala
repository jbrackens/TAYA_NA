...
object View03WalletTransfersProjectionHandler {
  ...
  private def withTransactionEvent(reason: TransactionReason): Option[(TransferType, TransferDescription)] = {
    ...
    reason match {
      case BetWon | PredictionWon                  => Some((ToWallet, Profit))
      case BetLost | PredictionLost                => Some((FromWallet, Profit))
      ...
      case FundsReservedForBet | FundsReservedForPrediction | BetVoided | PredictionVoided | BetPushed |
          PredictionPushed | BetCancelled | PredictionCancelled | BetResettled | PredictionResettled |
          WithdrawalCancelled | FundsReservedForWithdrawal =>
        None
    }
  }
}
