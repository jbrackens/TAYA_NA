...
final class CancelledPredictionData(
    predictionReadModels: PredictionQueryService,
    puntersFinder: PuntersFinder)(implicit ec: ExecutionContext)
    extends ReportDataProvider[PredictionCancelledRow] {

  override protected def provideData(reportingPeriod: ReportingPeriod): Future[Seq[PredictionCancelledRow]] =
    for {
      cancelledOrders <- predictionReadModels.listAllOrders(status = Some("cancelled"))
      contexts <- predictionReadModels.predictionContextsForOrderIds(cancelledOrders.map(_.orderId).toSet)
      marketViews <- predictionReadModels
        .listMarkets()
        .map(markets => markets.map(market => market.marketId -> market).toMap)
      rows <- cancelledOrders.toList
        .filter(order => within(reportingPeriod, OffsetDateTime.parse(order.updatedAt)))
  ...
  private def buildRow(
      ...
      context: Option[phoenix.prediction.infrastructure.PredictionOrderContextView],
      marketView: Option[PredictionMarketView]): Future[PredictionCancelledRow] =
    puntersFinder.find(PunterId(order.punterId)).map { punterProfile =>
      PredictionCancelledRow(
        gamingDate = DateField(reportingPeriod.periodStart),
        transactionTime = TimeField(context.flatMap(_.settledAt).map(OffsetDateTime.parse).getOrElse(OffsetDateTime.parse(order.updatedAt))),
        ...
        reasonForCancellation = StringField(context.flatMap(_.settlementReason).filter(_.trim.nonEmpty).getOrElse("Cancelled")))
    }
