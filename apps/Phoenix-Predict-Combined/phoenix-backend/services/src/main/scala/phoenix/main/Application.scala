...
import phoenix.prediction.infrastructure.ComposedPredictionReadModelService
import phoenix.prediction.infrastructure.SlickPredictionProjectionPersistenceService
import phoenix.prediction.infrastructure.SlickPredictionQueryPersistenceService
...
trait Application {
  ...
  object RootBehavior {
    ...
    def startApplication(
      ...
      val predictionQueries = new SlickPredictionQueryPersistenceService(dbConfig)
      val predictionProjection = new SlickPredictionProjectionPersistenceService(dbConfig)
      val predictionPersistence =
        new ComposedPredictionReadModelService(
          predictionQueries,
          predictionProjection,
          transactionalAudit = predictionProjection.lifecycleAuditIsTransactional)
      Await.result(predictionProjection.syncSeedData(), 10.seconds)
      ...
      val phoenixRestServer =
        PhoenixRestServer.create(
          ...
          predictionReadModels = predictionPersistence,
          predictionQueryService = predictionQueries,
          predictionProjectionService = Some(predictionProjection),
          predictionHistoryLookup = predictionQueries,
