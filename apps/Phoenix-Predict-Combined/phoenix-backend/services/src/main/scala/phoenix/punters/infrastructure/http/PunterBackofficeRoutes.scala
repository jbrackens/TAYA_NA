...
import phoenix.prediction.infrastructure.PredictionPunterFinancialSummary
...
final class PunterBackofficeRoutes(
  ...
  private def getTransactions =
    adminEndpoint.get
      ...
      .in(enumQueryWithAllValuesAsDefault[WalletProduct]("filters.product"))
      .in(EndpointInputs.pagination.queryParams)
  ...
  private val financialSummaryRoute = financialSummary.serverLogic { _ => punterId =>
    wallets
      ...
      .value
      .flatMap {
        case Left(_: WalletNotFoundError) =>
          Future.successful(Left(ErrorResponse.tupled(StatusCode.NotFound, PresentationErrorCode.WalletNotFound)))
        case Right(fs) =>
          predictionHistoryLookup
            .predictionFinancialSummaryForPunter(punterId.value)
            .map { predictionSummary =>
              Right(
                FinancialSummaryResponse(
                  currentBalance = fs.currentBalance,
                  openedBets = fs.openedBets,
                  pendingWithdrawals = fs.pendingWithdrawals,
                  lifetimeDeposits = fs.lifetimeDeposits,
                  lifetimeWithdrawals = fs.lifetimeWithdrawals,
                  netCash = fs.netCash,
                  productBreakdown = FinancialSummaryProductBreakdown(
                    sportsbook = ProductExposureSummary(
                      openExposure = sportsbookOpenExposure(fs.openedBets, predictionSummary)),
                    prediction = PredictionProductSummary(
                      openExposure = predictionSummary.openExposure,
                      openOrders = predictionSummary.openOrders,
                      settledOrders = predictionSummary.settledOrders,
                      cancelledOrders = predictionSummary.cancelledOrders))))
            }
      }
  }
  ...
  val getTransactionsRoute = getTransactions.serverLogic { _ =>
    {
      case (punterId, timeRange, reasons, products, pagination) =>
        ...
        wallets
          ...
          .flatMap(result =>
            enrichWalletTransactions(result.data).map(enriched => Right(result.copy(data = enriched))))
    }
  }
  ...
  private val exportTransactionsRoute = exportTransactions.serverLogic { _ =>
    {
      case (punterId, timeRange, reasons, products) =>
        val query = WalletTransactionsQuery(WalletId.deriveFrom(punterId), timeRange, reasons, products)
        val source =
          wallets
            .allWalletTransactions(query)
            .mapAsync(1)(transaction => enrichWalletTransaction(transaction))
  ...
  private def enrichWalletTransactions(transactions: Seq[WalletTransaction])(implicit
      ec: ExecutionContext): Future[Seq[WalletTransactionView]] = {
    val predictionOrderIds = transactions.flatMap { transaction =>
      transaction.betId
      ...
      .map(contexts =>
        transactions.map(transaction =>
          WalletTransactionView.fromWalletTransaction(
            transaction,
            transaction.betId.flatMap(id => contexts.get(id.value)))))
  }

  private def enrichWalletTransaction(transaction: WalletTransaction)(implicit
      ec: ExecutionContext): Future[WalletTransactionView] =
    transaction.betId
      .filter(_ => WalletProduct.fromReason(transaction.reason) == WalletProduct.Prediction)
      .map(_.value) match {
      case None =>
        Future.successful(WalletTransactionView.fromWalletTransaction(transaction))
      case Some(orderId) =>
        predictionHistoryLookup
          .predictionContextForOrderId(orderId)
          .map(context => WalletTransactionView.fromWalletTransaction(transaction, context))
    }

  private def sportsbookOpenExposure(
      totalOpenedBets: RealMoney,
      predictionSummary: PredictionPunterFinancialSummary): RealMoney = {
    val sportsbookExposure = totalOpenedBets - predictionSummary.openExposure
    if (sportsbookExposure.value.amount >= 0) sportsbookExposure else RealMoney(0)
  }
