...
class FixtureChangedEventConsoleOutput extends AkkaStreamlet {

  val in = AvroInlet[FixtureChangedEvent]("in")
  ...
  override def createLogic(): AkkaStreamletLogic =
    new RunnableGraphStreamletLogic() {
      def runnableGraph(): RunnableGraph[_] =
        plainSource(in).to(Sink.foreach { elem => log.info(s"${classOf[FixtureChangedEvent].getSimpleName} -> $elem") })
    }
}
