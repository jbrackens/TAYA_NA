...
if [[ $# -ne 4 ]]; then
  echo "Usage:"
  echo "$(basename $0) <local-chart-name> <ecr-target-chart-repository> <docker-image-tag> <cluster-id>"
  echo
  echo "Example:"
  echo "$(basename $0) phoenix-backend 259420793117.dkr.ecr.eu-west-1.amazonaws.com/phoenix-backend-helm-chart 0.0.0-1234-16d85180 virginia-dev"
  echo
  echo "In the example, the chart will be published as:"
  echo "259420793117.dkr.ecr.eu-west-1.amazonaws.com/phoenix-backend-helm-chart:0.0.0-1234-16d85180-virginia-dev"
  exit 1
fi

local_chart_name=$1
ecr_target_chart_repository=$2
image_tag=$3
cluster=$4
...
# the problem is that we need a few changes on the chart before the release.
...
yq eval-all '. as $item ireduce ({}; . * $item)' \
  --inplace values.yaml \
  envs/eks-${cluster}_phoenix.yaml
...
yq eval ".version = \"$image_tag-$cluster\"" --inplace Chart.yaml
full_chart_version=$(yq eval '.version' Chart.yaml)
full_ecr_target_chart_id=$ecr_target_chart_repository:$full_chart_version
