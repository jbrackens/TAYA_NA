...
func registerSettlementRoutes(mux *stdhttp.ServeMux, svc *prediction.Service) {
	...
	mux.Handle("/api/v1/admin/markets/", httpx.Handle(func(w stdhttp.ResponseWriter, r *stdhttp.Request) error {
		...
		// GAP-20 slice 2/3: read + configure a market's required eligibility tags.
		// Both verbs are handled here, ABOVE the resolution dual-control guard and
		// the POST-only gate, so tag config carries its own accurate messaging — it
		// is an audited CONFIG action, not a dual-control resolution action, and must
		// not borrow that guard's "resolution (dual-control)" error text. markets:edit
		// is already enforced by the top-level gate for this whole handler.
		if len(parts) == 2 && strings.EqualFold(strings.TrimSpace(parts[1]), "eligibility-tags") {
			...
			case stdhttp.MethodPost:
				// Single POST with {tagId, op:"add"|"remove"} — the shared handler is
				// POST-only for multi-part paths, so remove rides here rather than
				// DELETE. An identified admin is required so the audit entry has an
				// actor (this is NOT dual-control; the message reflects that).
				actor := userIDFromRequest(r)
				if actor == "" {
					return httpx.Forbidden("an identified admin is required to change market eligibility tags (audited action)")
				}
				var body struct {
				...
				case "", "add":
					...
					recordProviderOpsAuditAction(actor, "market.eligibility_tags_changed", parts[0], map[string]any{
