...
func (s *PostgresResponsibleGamblingService) ensureSchema() error {
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
		...
		// Bet (slice 1), deposit (slice 2), loss (slice 3) limits.
...
func (s *PostgresResponsibleGamblingService) SetLossLimit(ctx context.Context, userID string, period string, amountCents int64) error {
	...
	return s.setLimitWithLoosenCooldown(ctx, "player_loss_limits", userID, period, amountCents)
}
...
func (s *PostgresResponsibleGamblingService) GetLossLimits(ctx context.Context, userID string) ([]LossLimit, error) {
	...
	// reflect the now-effective limit; a still-future pending is surfaced below.
	if _, err := s.db.ExecContext(dctx, `
UPDATE player_loss_limits
SET limit_cents = pending_limit_cents, pending_limit_cents = NULL,
    pending_activates_at = NULL, updated_at = NOW()
WHERE user_id = $1 AND pending_activates_at IS NOT NULL AND pending_activates_at <= NOW()`, userID); err != nil {
		return nil, err
...
func (s *PostgresResponsibleGamblingService) lossLimitDenial(ctx context.Context, userID string) (string, error) {
	...
	defer cancel()
	// Effective limit (GAP-63): staged loosening does not apply until matured.
	rows, err := s.db.QueryContext(dctx, `SELECT period,
       CASE WHEN pending_activates_at IS NOT NULL AND pending_activates_at <= NOW()
            THEN pending_limit_cents ELSE limit_cents END
FROM player_loss_limits WHERE user_id = $1`, userID)
	if err != nil {
		return "", err
	}
	type lossCap struct {
