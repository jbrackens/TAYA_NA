import { isEmpty, orderBy, find, get, first } from "lodash";
import qs from "qs";
import { ParsedQuery, ParsedQueryPagination } from "../../types/crud";
import { markets } from "../../mocked_data/markets";
...
export default {
  async list(req: any, res: any) {
    ...
    if (!isEmpty(sorting) || !isEmpty(pagination)) {
      ...
      return res.status(200).send({
        data: resolvePagination(
          orderBy(markets, [get(sorting, "field")], [get(sorting, "order")]),
          parsedPagination,
        ),
    ...
    return res.status(200).send({
      data: resolvePagination(markets),
      totalCount: markets.length,
      ...DEFAULT_PAGINATION,
    });
  },
  ...
  async details(req: any, res: any) {
    ...
    const basic = find(markets, (item) => item.market.marketId === parsedId);
