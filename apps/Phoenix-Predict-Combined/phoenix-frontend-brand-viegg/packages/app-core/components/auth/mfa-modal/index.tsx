...
import { useRequestVerification } from "../../../services/go-api";
...
const MfaModalComponent: React.FC<MfaModalComponentProps> = ({
  ...
  useEffect(() => {
    if (requestVerificationMutation.isSuccess && requestVerificationMutation.data) {
      setVerificationId(requestVerificationMutation.data.verificationId);
    }
  }, [requestVerificationMutation.isSuccess, requestVerificationMutation.data]);
        ...
        <CoreForm {...formItemLayout} name="mfaForm" form={form}>
          <InputsContainer>
            ...
            >
              <CoreSpin
                spinning={
                  requestVerificationMutation.isLoading || isProcessingVerification
                }
              >
                <CodeInput
                  name="code"
          ...
          {(requestVerificationMutation.isLoading ||
            requestVerificationMutation.error) && (
            <Row
              justify="center"
              align="middle"
              gutter={[32, 32]}
              role={requestVerificationMutation.isLoading ? "loading" : "error"}
            >
              {requestVerificationMutation.error && (
                <Col span={24}>
