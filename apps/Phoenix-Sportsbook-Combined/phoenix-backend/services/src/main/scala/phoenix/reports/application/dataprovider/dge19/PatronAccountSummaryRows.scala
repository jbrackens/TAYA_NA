...
private[dge19] object PatronAccountSummaryRows {
  ...
  def buildReportRow(
      ...
      predictionSummary: PredictionPunterReportSummary,
    ...
    PatronAccountSummaryReportRow(
      ...
      transfersToPrediction = MoneyField(predictionSummary.transfersToPrediction.amount),
      cancelledPredictionOrders = MoneyField(predictionSummary.cancelledPredictionOrders.amount),
      transfersFromPrediction = MoneyField(predictionSummary.transfersFromPrediction.amount),
      endingPredictionExposure = MoneyField(predictionSummary.endingPredictionExposure.amount),
      patronPredictionWinLoss = MoneyField(predictionSummary.patronPredictionWinLoss.amount),
