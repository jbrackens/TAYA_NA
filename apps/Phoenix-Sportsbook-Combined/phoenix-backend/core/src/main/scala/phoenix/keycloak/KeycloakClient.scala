...
class KeycloakClient(authServerUrl: String, adminUserName: String, adminPassword: String) {
  ...
  private def createRealm(realmName: String): RealmResource = {
    val createRealmRequest = {
      ...
      // Local development runs over http://localhost, so require no SSL for realm endpoints.
      request.setSslRequired("NONE")
  ...
  private def createRoles(realm: RealmResource): Unit = {
    ...
    List(
      new RoleRepresentation("admin", "Role for backoffice admins", scopeParamRequired),
      new RoleRepresentation("trader", "Role for prediction operations traders", scopeParamRequired),
      new RoleRepresentation("operator", "Role for prediction operations oversight", scopeParamRequired))
      .foreach(realm.roles().create)
  }
