...
import UsersPredictionOrdersContainer from "../../prediction-orders";
// const {
//   SHOW_FOR_SUBMISSION,
...
export const ActivityDetails = ({ id }: ActivityDetailsProps) => {
  ...
  const onTabClick = (tabName: string) => {
    const queryWithoutPageAndLimit = omit(router.query, [
      "p",
      "limit",
      "predictionP",
      "predictionLimit",
      ...
      >
        ...
        <TabPane
          tab={
            <span>
              <FundOutlined />
              {t("HEADER_PREDICTION_ORDERS")}
            </span>
          }
          key="predictionOrders"
        >
          <UsersPredictionOrdersContainer id={id} />
        </TabPane>
        <TabPane
          tab={
            <span>
