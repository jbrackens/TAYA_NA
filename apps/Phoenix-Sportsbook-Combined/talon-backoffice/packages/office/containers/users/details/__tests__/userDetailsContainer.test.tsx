...
jest.mock("../../../../utils/auth", () => ({
  ...
  resolveToken: jest.fn().mockReturnValue("mock-admin-token"),
  validateAndCheckEligibility: jest.fn().mockReturnValue(true),
}));
...
describe("user details container test", () => {
  ...
  test("UsersDetailsContainer: prediction orders tab content should be visible when activityDetails query equals to predictionOrders", async () => {
    mockedRouter.mockReturnValue({
      route: "/",
      pathname: "",
      query: {
        activityDetails: "predictionOrders",
      },
      asPath: "",
      push: jest.fn,
    });
    ...
    expect(
      screen.queryByText("page-prediction-ops:ORDERS_TABLE_ORDER"),
    ).toBeInTheDocument();
  });
