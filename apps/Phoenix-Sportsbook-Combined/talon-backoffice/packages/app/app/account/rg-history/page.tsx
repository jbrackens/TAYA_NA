...
export default function RGHistoryPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const grouped: GroupedHistory = {
          limits: history.history.filter(h =>
          ...
          exclusions: history.history.filter(h => h.limitType === 'self_exclusion'),
        };
        setGrouped(grouped);
      } catch (err: unknown) {
  ...
  const HistoryTable = ({ title, items }: { title: string; items: LimitHistoryItem[] }) => (
                ...
                <tr key={idx}>
                  <td>
                    <span className="rg-type">
                      {item.limitType === 'deposit_limit' ? 'Deposit Limit' :
                       item.limitType === 'stake_limit' ? 'Stake Limit' :
                       item.limitType === 'session_limit' ? 'Session Limit' :
                       item.limitType === 'cool_off' ? 'Cool-off' :
                       item.limitType === 'self_exclusion' ? 'Self-Exclusion' :
                       item.limitType}
                    </span>
                  </td>
                  <td>
                    {item.oldValue !== null && item.oldValue !== undefined
                      ? typeof item.oldValue === 'boolean'
                        ? item.oldValue ? 'Enabled' : 'Disabled'
                        : `$${item.oldValue}`
                      : '—'}
                  </td>
                  <td>
                    {item.newValue !== null && item.newValue !== undefined
                      ? typeof item.newValue === 'boolean'
                        ? item.newValue ? 'Enabled' : 'Disabled'
                        : `$${item.newValue}`
                      : '—'}
                  </td>
              ...
              grouped.exclusions.length === 0 && (
                <div className="rg-no-history">
                  ...
                  <div className="rg-no-history-desc">
                    You haven't set any responsible gaming limits yet.
                    Visit the responsible gaming page to get started.
                  </div>
...
const rgHistoryStyles = `
  ...
  .rg-header h1 { font-size: 28px; font-weight: 800; color: #e2e8f0; margin-bottom: 4px; }
  .rg-header p { font-size: 14px; color: #64748b; }
  ...
  .rg-back:hover { border-color: #f97316; color: #f97316; }
  ...
  .rg-empty {
    ...
    background: #161a32; border-radius: 8px;
  }
  ...
  .rg-no-history-desc {
    font-size: 13px; color: #64748b; max-width: 400px; margin-bottom: 20px;
    line-height: 1.5;
  }

  .rg-link-btn {
    display: inline-block; padding: 10px 20px; background: #f97316;
