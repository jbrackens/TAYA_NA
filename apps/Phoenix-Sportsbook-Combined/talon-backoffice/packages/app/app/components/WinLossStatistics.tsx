...
export default function WinLossStatistics({ userId }: WinLossStatisticsProps) {
  ...
  const getFilteredBets = (): UserBet[] => {
    ...
    return bets.filter((bet) => {
      ...
      const daysDiff = (now.getTime() - betDate.getTime()) / (1000 * 60 * 60 * 24);
  ...
  if (loading) {
    return (
      <div style={containerStyle}>
        <div style={{ color: '#64748b', fontSize: '13px' }}>Loading statistics...</div>
      </div>
    );
  }
    ...
    <div style={containerStyle}>
      ...
      <div style={statsGridStyle}>
        ...
        <div style={cardStyle}>
          ...
          <div style={cardValueStyle}>{formatCurrency(stats.totalReturned)}</div>
        </div>
