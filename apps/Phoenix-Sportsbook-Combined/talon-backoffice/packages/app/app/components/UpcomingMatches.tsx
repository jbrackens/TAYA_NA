...
export const UpcomingMatches: React.FC<UpcomingMatchesProps> = ({ limit = 50 }) => {
  ...
  useEffect(() => {
    ...
    const loadUpcomingMatches = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : 'Failed to load upcoming matches';
          setError(message);
        }
      } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      {Object.entries(matchesByGroup).map(([sport, matches]) => (
        <div key={sport} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <h3 style={{ fontSize: '16px', fontWeight: 600, color: '#e2e8f0', margin: 0, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            {sport}
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
            {matches.map((match: Event) => (
              <div key={match.eventId} style={{ position: 'relative' }}>
                <a href={`/match/${match.fixtureId || match.eventId}`} style={{ textDecoration: 'none' }}>
                <div style={{
                  position: 'absolute', top: '12px', right: '12px',
                  backgroundColor: match.status === 'in_play' ? 'rgba(34,197,94,0.2)' : 'rgba(74, 126, 255, 0.2)',
                  color: match.status === 'in_play' ? '#22c55e' : '#4a7eff',
                  padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 600,
                  zIndex: 1,
                }}>
                  {match.status === 'in_play' ? 'LIVE' : getCountdownText(match.startTime)}
                </div>
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{
                      ...
                      backgroundColor: match.status === 'in_play' ? '#22c55e' : '#f97316',
                      animation: match.status === 'in_play' ? 'pulse 2s infinite' : undefined,
                    }} />
