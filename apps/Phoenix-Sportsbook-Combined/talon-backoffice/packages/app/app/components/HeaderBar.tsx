...
import { selectOddsFormat, setOddsFormat, DisplayOddsEnum } from '../lib/store/settingsSlice';
...
import type { Event, Sport, League } from '../lib/api/events-client';
...
export const HeaderBar: React.FC = () => {
  ...
  const cycleLang = useCallback(() => {
    ...
    try { localStorage.setItem('phoenix_language', next); } catch {}
  }, [i18n]);

  const cycleOddsFormat = useCallback(() => {
    const order = [DisplayOddsEnum.DECIMAL, DisplayOddsEnum.AMERICAN, DisplayOddsEnum.FRACTIONAL];
  ...
  useEffect(() => {
    ...
    if (searchOpen && quickSports.length === 0) {
      getSports().then(setQuickSports).catch(() => {});
    }
  ...
  useEffect(() => {
    ...
    debounceRef.current = window.setTimeout(async () => {
      try {
        // Search across all sports — the Go backend filters by query param if supported,
        // otherwise we do client-side filtering on homeTeam/awayTeam
      ...
      <nav className="ps-topbar-tabs">
        {HEADER_TABS.map((tab) => (
          <a
            key={tab.href}
            href={tab.href}
            className={`ps-topbar-tab ${pathname === tab.href ? 'active' : ''}`}
          >
            {tab.label}
          </a>
        ))}
      ...
      <div className="ps-topbar-right">
        ...
        <button
          ...
          style={{
            ...
            color: '#f97316', cursor: 'pointer', letterSpacing: '0.03em',
        ...
        {/* Search */}
        <button className="ps-topbar-search" onClick={() => setSearchOpen(!searchOpen)}>
          <SearchIcon />
        ...
        {isAuthenticated ? (
          <>
            ...
            <a href="/cashier" className="ps-wallet-badge">
              <WalletIcon />
              <span>${typeof walletBalance === 'number' ? walletBalance.toFixed(2) : '0.00'}</span>
            </a>
          ...
          <>
            <a href="/auth/login" className="ps-btn-login">{t('LOGIN_LINK')}</a>
            <a href="/auth/login?mode=register" className="ps-btn-signup">{t('SIGN_UP_LINK')}</a>
          </>
        )}
      </div>
    </header>
            ...
            {searchQuery.trim().length < 2 ? (
                      ...
                      {quickSports.map((s: Sport) => (
                        <a
                          ...
                          style={{
                            ...
                            background: '#161a35', border: '1px solid #1e2243',
                            color: '#e2e8f0', textDecoration: 'none', cursor: 'pointer',
                          }}
                        >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 400, overflowY: 'auto' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>
                  {searchResults.length} {searchResults.length === 1 ? t('SEARCH_RESULT') : t('SEARCH_RESULT_PLURAL')}
                </div>
                {searchResults.map((event: Event) => (
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      background: event.status === 'in_play' ? 'rgba(34,197,94,0.1)' : 'rgba(249,115,22,0.1)',
                      color: event.status === 'in_play' ? '#22c55e' : '#f97316',
                    }}>
                      {event.status === 'in_play' ? t('SEARCH_STATUS_LIVE') : event.status === 'finished' ? t('SEARCH_STATUS_FT') : t('SEARCH_STATUS_UPCOMING')}
                    </div>
                  </a>
                ))}
              </div>
              ...
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                <div style={{ color: '#94a3b8' }}>{t('SEARCH_NO_RESULTS', { query: searchQuery })}</div>
