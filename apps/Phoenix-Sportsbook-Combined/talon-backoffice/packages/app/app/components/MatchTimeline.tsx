...
export const MatchTimeline: React.FC<MatchTimelineProps> = ({
  ...
  const handleFixtureUpdate = useCallback((message: WsMessage) => {
    ...
    setIncidents((prev) => {
      ...
      const existingIds = new Set(prev.map((i: MatchTrackerIncident) => i.incidentId));
      const merged = [
        ...newIncidents.filter((i: MatchTrackerIncident) => !existingIds.has(i.incidentId)),
        ...prev,
      ];
      ...
      {incidents.length > 0 ? (
        <div style={{
          ...
          {incidents.map((incident) => (
            <div key={incident.incidentId} style={{
              display: 'flex',
              gap: '16px',
              padding: '12px',
