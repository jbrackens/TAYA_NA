...
jest.mock("../../../../services/go-api", () => ({
  useChangePassword: jest.fn(() => ({ ...mockChangePasswordMutation })),
  useRequestVerification: jest.fn(() => ({ ...mockChangePasswordMutation })),
}));
...
describe("password modal test", () => {
  ...
  test("personal details form: should show error when error is true", async () => {
    const { useChangePassword } = require("../../../../services/go-api");
    (useChangePassword as jest.Mock).mockReturnValue({ ...mockChangePasswordMutation, error: { payload: { errors: [{ errorCode: "fake error" }] } } });
