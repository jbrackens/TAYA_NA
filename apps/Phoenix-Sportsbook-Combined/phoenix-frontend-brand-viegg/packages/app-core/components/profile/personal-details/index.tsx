...
import { useTermsCurrent } from "../../../services/go-api/terms/terms-hooks";
...
const PersonalDetailsComponent: React.FC<PersonalDetailsComponentProps> = ({
  ...
  const { data: termsCurrent } = useTermsCurrent();
  const termsToAcceptVersion: string | null = termsCurrent?.version ?? null;
  ...
  const onFinish = (values: any) => {
    ...
    if (currentModal === ModalTypeEnum.CHANGE_PHONE) {
      request.phone = values.phoneNumber;
    }

    if (currentModal === ModalTypeEnum.CHANGE_ADDRESS) {
      request.address = {
        address_line: values.address?.addressLine,
