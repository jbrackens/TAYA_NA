...
const ApiWrapper: React.FC<ApiWrapperProps> = ({
  ...
  const {
    isConnectionOpen: isWsConnectionOpen,
    openConnection,
    closeConnection,
  } = websocketApi;
  ...
  const shouldEnableRealtime =
    !disableWebsocket && Boolean(token) && isRefreshTokenValid;
  ...
  useEffect(() => {
    if (shouldEnableRealtime && !isWsConnectionOpen) {
      openConnection();
      return;
    }
  ...
  }, [closeConnection, isWsConnectionOpen, openConnection, shouldEnableRealtime]);
