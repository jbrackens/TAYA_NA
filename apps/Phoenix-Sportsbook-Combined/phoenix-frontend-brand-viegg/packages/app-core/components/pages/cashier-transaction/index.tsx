...
import { usePaymentTransaction } from "../../../services/go-api";
...
function CashierTransaction() {
  ...
  const { data: transactionData, refetch: refetchTransaction, error: transactionError } = usePaymentTransaction(txId || "");
  ...
  useEffect(() => {
    ...
    if (
      transactionData.status === PaymentStatusEnum.PENDING ||
      transactionData.status === PaymentStatusEnum.INITIATED
    ) {
  ...
  useEffect(() => {
    if (transactionError) setIsLoading(false);
  }, [transactionError]);
