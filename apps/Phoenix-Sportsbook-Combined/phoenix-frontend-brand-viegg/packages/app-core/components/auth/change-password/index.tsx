...
import { useResetPasswordByToken } from "../../../services/go-api";
...
const ChangePasswordComponent: React.FC = () => {
  ...
  const [requestErrors, setRequestErrors] = useState<Array<{ errorCode: string }>>();
  ...
  const resetMutation = useResetPasswordByToken(token || "");
  ...
  useEffect(() => {
    if (!resetMutation.isSuccess) {
      setRequestErrors((resetMutation.error as AppError | undefined)?.payload?.errors);
      return;
    }
    setRequestErrors([]);
    setVerificationSuccess(true);
    message.success(t("SUCCESS_MESSAGE"));
    router.push(`/sports/home?showModal=${ModalTypeEnum.LOGIN}`);
