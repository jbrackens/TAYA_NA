import { resolveFeatureApiBaseUrl, useApi } from "../api-service";
...
jest.mock("next/config", () => ({
  default: () => ({
    publicRuntimeConfig: {
      ...
      CANONICAL_GATEWAY_ENDPOINT: "http://canonical.gateway.endpoint",
    },
  }),
}));
...
describe("app-core api service contract", () => {
  beforeEach(() => {
  ...
  test("prefers canonical gateway endpoint for sportsbook feature clients", () => {
    expect(resolveFeatureApiBaseUrl()).toBe("http://canonical.gateway.endpoint");
  });
});
